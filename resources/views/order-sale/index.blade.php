      ...
      <div class="summary-model">
        ...
        <div class="sm-col">
          <div class="sm-row"><label>Tax</label>
            <div class="pair-2">
              <input id="fTaxPerc" value="{{ $__isSecondaryDb ? '0' : '3.0' }}"{{ $__isSecondaryDb ? '' : ' readonly' }}>
...
const IS_SECONDARY_DB = !!CFG.isSecondaryDb;
...
function defaultTaxPercText() {
  return IS_SECONDARY_DB ? '0' : '3.0';
}
...
function loadOrder(ordno) {
  ...
  api(`/api/order-sale/load-order?order_no=${encodeURIComponent(ordno)}`)
    .then(d => {
      ...
      // Keep linked-order exchange only as an amount on the sale.
      // Do not pull the original order exchange detail rows into order-sale,
      // otherwise they get treated like new exchange entries again.
      exchRows = [];
...
function doRecalc(rparm = 0, source = '') {
  ...
  if (['fCcardAmt', 'fChqAmt', 'fCashLess', 'credit'].includes(source)) {
    receivedTouched = false;
  }
...
['fCcardAmt','fChqAmt','fCashLess'].forEach(id => {
  $(id).addEventListener('input', () => doRecalc(1, id));
});
