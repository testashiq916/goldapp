...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .action-options{display:flex;gap:12px;margin-bottom:4px;margin-top:2px}
  .action-option{display:flex;align-items:center;gap:7px;padding:8px 14px;border:1.5px solid #e2e8f0;border-radius:10px;cursor:pointer;font-size:13px;font-weight:600;color:#475569;transition:all .15s;user-select:none}
  .action-option:hover{border-color:#93c5fd;background:#eff6ff;color:#1d4ed8}
  .action-option input[type=radio]{width:16px;height:16px;accent-color:#3b82f6;cursor:pointer;margin:0}
  .action-option.selected-edit{border-color:#3b82f6;background:#eff6ff;color:#1d4ed8}
  .action-option.selected-cancel{border-color:#ef4444;background:#fef2f2;color:#b91c1c}
  ...
  tbody tr.cancelled{background:#fff7ed}
  tbody tr.cancelled:hover{background:#ffedd5}
  .badge{display:inline-flex;align-items:center;justify-content:center;min-width:72px;padding:3px 8px;border-radius:999px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.3px}
...
<div class="card">
  ...
  <div class="card-body">
    <div class="field-group">
      <label>{{ !empty($quotationMode) ? 'Quotation Number' : 'Bill Number' }}</label>
      <div class="input-row">
        <input type="text" id="billNo" maxlength="30" autocomplete="off" placeholder="{{ !empty($quotationMode) ? 'Enter quotation no...' : 'Enter bill no...' }}" style="text-transform:uppercase">
    ...
    @if(!empty($showActionChoice) && $actionMode !== 'reprint')
    <div class="field-group" style="margin-bottom:12px">
      <label>Action</label>
      <div class="action-options">
        <label class="action-option selected-edit" id="optEdit">
          <input type="radio" name="actionOpt" value="edit" checked> Edit
        </label>
        <label class="action-option" id="optCancel">
          <input type="radio" name="actionOpt" value="cancel"> Cancel
        </label>
      </div>
    </div>
...
<div class="modal-bg" id="helpModal">
  <div class="modal">
    <div class="modal-head">
      <input type="text" id="helpQuery" placeholder="Search bill no or customer...">
...
<script>
const SEARCH_API = "{{ url('/api/sales-bill/edit-search') }}";
const RESOLVE_API = "{{ url('/api/sales-bill/action-resolve') }}";
const INIT_ACTION_MODE = @json($actionMode);
const QUOTATION_MODE = @json(!empty($quotationMode));
const csrf = document.querySelector('meta[name="csrf-token"]').content;

function $(id){ return document.getElementById(id); }
function getActionMode(){
  const sel = document.querySelector('input[name="actionOpt"]:checked');
  return sel ? sel.value : INIT_ACTION_MODE;
}
// Highlight selected radio option
document.querySelectorAll('input[name="actionOpt"]').forEach(r => {
  r.addEventListener('change', function(){
    if ($('optEdit')) $('optEdit').className = 'action-option' + (this.value === 'edit' ? ' selected-edit' : '');
    if ($('optCancel')) $('optCancel').className = 'action-option' + (this.value === 'cancel' ? ' selected-cancel' : '');
  });
});
// Set initial state from server-provided mode
if (INIT_ACTION_MODE === 'cancel') {
  const r = document.querySelector('input[name="actionOpt"][value="cancel"]');
  if (r) {
    r.checked = true;
    if ($('optEdit')) $('optEdit').className = 'action-option';
    if ($('optCancel')) $('optCancel').className = 'action-option selected-cancel';
  }
}
...
async function searchHelp(){
  ...
  if (QUOTATION_MODE) params.set('qtn', '1');
  const data = await fetchJson(`${SEARCH_API}?${params.toString()}`);
  const rows = data.rows || [];
  $('helpRows').innerHTML = rows.map((r, i) => `
    <tr data-idx="${i}" class="${String(r.status||'').toLowerCase()==='cancelled' ? 'cancelled' : ''}">
      <td>${r.billno || ''}</td>
      <td>${r.tdate || ''}</td>
      <td>${r.custname || ''}</td>
...
async function openSelectedBill(){
  ...
  if (!billNo || !tdate) {
    setStatus('Bill no and date are required.');
    return;
  }
