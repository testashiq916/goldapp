  ...
  <table>
    <thead>
      <tr>
        ...
        <th colspan="2" class="grp-dep">Deposit</th>
        <th colspan="2" class="grp-wit">Withdraw</th>
        <th colspan="2" class="grp-bal">Balance</th>
      </tr>
      <tr>
        <th class="num grp-dep">Gross Wgt</th><th class="num grp-dep">Net Wgt</th>
        <th class="num grp-wit">Gross Wgt</th><th class="num grp-wit">Net Wgt</th>
        <th class="num grp-bal">Gross Wgt</th><th class="num grp-bal">Net Wgt</th>
      </tr>
    </thead>
    ...
    <tfoot>
      <tr>
        ...
        <td class="num" id="totDepWgt">0.000</td><td class="num" id="totDepNet">0.000</td>
        <td class="num" id="totWitWgt">0.000</td><td class="num" id="totWitNet">0.000</td>
        <td class="num" id="totBalWgt">0.000</td><td class="num" id="totBalNet">0.000</td>
      </tr>
    </tfoot>
  </table>
</div>
<div class="empty" id="emptyState">Set filters and click <strong>Show</strong> to view the summary.</div>
</div>
<script>
const API=@json(url('/api/deposit-reports/wgt-amt-summary/data'));
...
document.getElementById('filterForm').addEventListener('submit',async e=>{
  ...
  try{
    ...
    document.getElementById('reportBody').innerHTML=rows.map(r=>`<tr>
      <td><strong>${esc(r.partyname)}</strong><br><small style="color:#5b7088">${esc(r.smithcode)}</small></td>
      <td class="num">${r.total_txns}</td>
      <td class="num pos">${n3(r.dep_wgt)}</td><td class="num pos">${n3(r.dep_net)}</td>
      <td class="num neg">${n3(r.wit_wgt)}</td><td class="num neg">${n3(r.wit_net)}</td>
    ...
    document.getElementById('totDepWgt').textContent=n3(t.dep_wgt);
    document.getElementById('totDepNet').textContent=n3(t.dep_net);
    document.getElementById('totWitWgt').textContent=n3(t.wit_wgt);
    document.getElementById('totWitNet').textContent=n3(t.wit_net);
