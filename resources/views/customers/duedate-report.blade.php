...
<div class="table-wrap"><table id="tbl">
  <thead><tr>
    <th>#</th><th>Code</th><th>Customer Name</th><th>Mobile</th><th>Telephone</th>
...
function load(){
  ...
  fetch('/api/customers/duedate-report?date1='+document.getElementById('date1').value
  ...
  .then(r=>r.json()).then(d=>{
    ...
    tbody.innerHTML=d.rows.map((r,i)=>{
      ...
      return `<tr class="${cls}">
        ...
        <td>${r.overdue?'<span style="color:#a72c2c;font-weight:700">OVERDUE</span>':''}</td>
      </tr>`;
    }).join('')
    +'<tr class="tfoot"><td colspan="5">Total ('+d.totals.count+')</td>'
    +'<td class="num">'+fmt(d.totals.tr+d.totals.tg)+'</td>'
    +'<td colspan="3"></td></tr>';
    var t=d.totals;
    document.getElementById('p_count').textContent='Customers: '+t.count;
    document.getElementById('p_tr').textContent='To Receive: ₹'+fmt(t.tr);
    document.getElementById('p_tg').textContent='To Give: ₹'+fmt(t.tg);
