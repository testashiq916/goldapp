...
<div class="table-wrap"><table id="tbl">
  <thead><tr>
    ...
    <th class="num">Bills</th><th class="num">Total Amount</th>
    <th class="num">Received</th><th class="num">Balance</th>
  </tr></thead>
...
function load(){
  ...
  fetch('/api/customers/summary?date1='+document.getElementById('date1').value
  ...
  .then(r=>r.json()).then(d=>{
    ...
    tbody.innerHTML=d.rows.map((r,i)=>`<tr>
      ...
      <td class="num">${fmt(r.total_amt)}</td>
      <td class="num">${fmt(r.total_rcvd)}</td>
      <td class="num bal">${fmt(r.total_bal)}</td>
    </tr>`).join('')
    +'<tr class="tfoot"><td colspan="4">Total ('+d.rows.length+' customers)</td>'
    +'<td class="num">'+d.totals.bill_count+'</td>'
    +'<td class="num">'+fmt(d.totals.total_amt)+'</td>'
    ...
    document.getElementById('p_amt').textContent='Total: ₹'+fmt(t.total_amt);
