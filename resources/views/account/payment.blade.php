...
<head>
    ...
    <style>
        ...
        .bal-box { display: inline-flex; align-items: center; gap: 8px; background: #edf4fc; border: 1px solid #c5d9ef; border-radius: 8px; padding: 6px 16px; font-size: 20px; font-weight: 800; letter-spacing:.2px }
        .bal-box .val { color: #0f4c81; font-size: 26px; font-weight: 800 }
        .bal-box .lbl { font-size: 14px; color: #6b7f9a; font-weight: 700; letter-spacing:.3px; text-transform: uppercase }
...
<div class="wrap">
    <h1>
        ...
        <div class="bal-box">
            ...
            <span id="cashBalVal" class="val"></span>
            <span id="cashBalAfter" style="display:none;margin-left:10px;color:#b91c1c;font-weight:800;font-size:18px">&rarr; <span id="cashBalAfterVal" style="font-size:22px"></span></span>
        </div>
        <span class="nav-row">
    ...
    <div class="sec">
        ...
        <div class="chk-row" style="margin-top:6px;">
            ...
            <label><input id="chkPrintRuff" type="checkbox"> Print Ruff Slip</label>
            <label><input id="chkPrintObCb" type="checkbox" checked> Print OB/CB</label>
        </div>
    </div>
    ...
    <div class="sec" style="margin-top:10px;">
        <div style="display:flex;gap:8px;align-items:center;margin-bottom:8px;">
            <h3 style="margin:0;flex:1;">Recent Payments</h3>
...
(() => {
    ...
    const BASE = @json(url('/api/accounts/payment'));
    ...
    function openPrintView(slno) {
        ...
        const qs = new URLSearchParams({
            slno: String(id),
            obcb: document.getElementById('chkPrintObCb').checked ? '1' : '0',
            slip: document.getElementById('chkPrintSlip').checked ? '1' : '0',
            ruff: document.getElementById('chkPrintRuff').checked ? '1' : '0',
        });
        const printUrl = @json(url('/accounts/payment-print')) + '?' + qs.toString();
    ...
    async function init() {
        try {
            ...
            // Show cash balance
            currentCashBalance = Number(initData.cashBalance || 0);
            document.getElementById('cashBalVal').textContent = currentCashBalance.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    ...
    function setMode(m) {
        ...
        if (m === 'A') {
            ...
            document.getElementById('title').textContent = 'Payment - Add';
        } else if (m === 'E') {
            document.getElementById('btnEdit').classList.add('active');
            document.getElementById('title').textContent = 'Payment - Edit';
        } else if (m === 'D') {
            document.getElementById('btnDelete').classList.add('active');
            document.getElementById('title').textContent = 'Payment - Delete';
        }
    }

    function resetForm() {
        document.getElementById('tdate').value = toIsoDate(initData.today || '');
        if (initData.cbAccounts && initData.cbAccounts.length > 0) {
            document.getElementById('cbcode').value = initData.defaultCbCode || initData.cbAccounts[0].accode;
        }
    ...
    // ── Cash/Bank change → show/hide cheque fields + update cash balance ──
    ...
    async function updateCashBalance() {
        const cbcode = (document.getElementById('cbcode').value || '').trim().toUpperCase();
        if (!cbcode) return;
        try {
            const d = await api({ action: 'load_account', accode: cbcode });
            currentCashBalance = Math.abs(d.balance || 0);
            document.getElementById('cashBalVal').textContent = currentCashBalance.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
            updateCashBalAfter();
        } catch (e) { /* silent */ }
    }

    function updateCashBalAfter() {
        const amt = parseFloat(document.getElementById('amount').value) || 0;
        const wrap = document.getElementById('cashBalAfter');
        const val  = document.getElementById('cashBalAfterVal');
        if (amt <= 0) { wrap.style.display = 'none'; return; }
        // Payment OUTFLOWS the cash/bank account, so the balance after this payment is current - amt.
        const projected = currentCashBalance - amt;
        val.textContent = projected.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        wrap.style.display = '';
    }
    document.getElementById('amount').addEventListener('input', updateCashBalAfter);
    ...
    // ── Load payment ──
    async function loadPayment(slno) {
        try {
            const d = await api({ action: 'load', slno: slno });
            ...
            document.getElementById('cashBalVal').textContent = Math.abs(r.cashBalance || 0).toFixed(2);
            updateCB();
            checkChequeVisibility();
    ...
    async function doSave() {
        ...
        try {
            ...
            if (!payload.pdc && (
                document.getElementById('chkPrintSlip').checked ||
                document.getElementById('chkPrintRuff').checked ||
                document.getElementById('chkPrintObCb').checked
            )) {
    ...
    async function refreshVchno() {
        try {
            ...
            initData.defaultCbCode = d.defaultCbCode || initData.defaultCbCode;
    ...
    document.getElementById('btnDelete').addEventListener('click', async () => {
        ...
        setMode('D');
        showStatus('Load a payment first, then click Delete again.', false);
    });
