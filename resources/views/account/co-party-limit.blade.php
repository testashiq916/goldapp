...
<head>
    ...
    <title>Party Credit Limit</title>
    <style>
        ...
        td input { width: 100%; box-sizing: border-box; height: 30px; border: 1px solid #bfd0e6; border-radius: 6px; padding: 0 8px; font-size: 12px; }
        ...
        <table id="limitTable">
            <thead>
                <tr>
                    ...
                    <th>Account Name</th>
                    <th style="width:160px;">Max Amount</th>
...
(() => {
    ...
    const BASE = @json(url('/api/accounts/master/co-party-limit'));
    ...
    function addRow(data = null) {
        ...
        tr.innerHTML = `
            <td><input data-f="accode" maxlength="20" value="${data ? escapeHtml(data.accode || data.code || '') : ''}"></td>
            <td><input data-f="acname" readonly value="${data ? escapeHtml(data.acname || data.name || '') : ''}"></td>
            <td><input data-f="maxamt" type="number" step="0.01" class="right" value="${data ? Number(data.maxamt || data.max_amount || 0) : ''}"></td>
            <td><button type="button" class="warn row-del" style="height:28px;padding:0 8px;font-size:11px;">X</button></td>
        `;
        limitBody.appendChild(tr);
        updateCount();
        if (!data) {
            tr.querySelector('[data-f="accode"]').focus();
        }
        return tr;
    }

    function updateCount() {
        document.getElementById('countLabel').textContent = limitBody.querySelectorAll('tr').length + ' records';
    }

    function getEntries() {
        return Array.from(limitBody.querySelectorAll('tr')).map(tr => {
            return {
                accode: (tr.querySelector('[data-f="accode"]').value || '').toUpperCase().trim(),
                acname: tr.querySelector('[data-f="acname"]').value || '',
                maxamt: Number(tr.querySelector('[data-f="maxamt"]').value || 0)
            };
        }).filter(r => r.accode);
    }
    ...
    async function loadExisting() {
        try {
            ...
            if (!limitBody.children.length) addRow();
            status('Loaded ' + (d.data || []).length + ' records', true);
    ...
    async function doSave() {
        ...
        if (!entries.length) { status('No entries to save', false); return; }
    ...
    limitBody.addEventListener('change', (e) => {
        if (e.target.matches('[data-f="accode"]')) {
            resolveAccountName(e.target);
        }
    });

    // Account code - uppercase on input
    limitBody.addEventListener('input', (e) => {
        if (e.target.matches('[data-f="accode"]')) {
    ...
    limitBody.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Enter' && e.target.matches('input')) {
            ...
            if (nextTd && nextTd.querySelector('input:not([readonly])')) {
                nextTd.querySelector('input:not([readonly])').focus();
                return;
            }
            const tr = e.target.closest('tr');
            const nextTr = tr.nextElementSibling;
            if (nextTr && nextTr.querySelector('input')) {
    ...
    // Row delete
    limitBody.addEventListener('click', (e) => {
        const b = e.target.closest('.row-del');
        if (b) { b.closest('tr').remove(); updateCount(); }
    });
    ...
    document.getElementById('acRows').addEventListener('dblclick', (e) => {
        ...
        document.getElementById('acModal').classList.remove('show');
        const maxInput = row.querySelector('[data-f="maxamt"]');
    ...
    document.getElementById('btnDeleteRow').addEventListener('click', () => {
        ...
        if (focused && limitBody.contains(focused)) {
            focused.remove();
            updateCount();
        } else {
