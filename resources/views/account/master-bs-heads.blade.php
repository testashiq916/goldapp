...
<body>
<div class="wrap">
    <h1 id="title">BS Heads</h1>
    <div id="status" class="status"></div>

    <div class="sec">
        <h3>BS Head Details</h3>
        <div class="grid">
            <label>Head Code</label>
            <input id="hcode" maxlength="10">

            <label>Head Name</label>
            <input id="hname" maxlength="50">
            ...
            <label>Expand</label>
            <div class="chk-line">
                <input id="chkExpand" type="checkbox"> Expand in reports
            </div>
        </div>
    </div>
    ...
    <div class="sec" style="margin-top: 10px;">
        <h3>Existing BS Heads</h3>
        <div class="list-wrap">
            <table>
                <thead><tr><th style="width:100px;">Code</th><th>Name</th><th style="width:60px;">Type</th><th style="width:50px;">Pos</th><th style="width:60px;">Expand</th></tr></thead>
...
(() => {
    ...
    const BASE = @json(url('/api/accounts/master/bs-heads'));
    ...
    function setMode(m) {
        ...
        if (m === 'A') {
            ...
            document.getElementById('title').textContent = 'BS Heads - Add';
            document.getElementById('hcode').disabled = false;
        } else if (m === 'E') {
            document.getElementById('btnEdit').classList.add('active');
            document.getElementById('title').textContent = 'BS Heads - Edit';
        } else if (m === 'D') {
            document.getElementById('btnDelete').classList.add('active');
            document.getElementById('title').textContent = 'BS Heads - Delete';
        }
    }

    function collectPayload() {
        return {
            hcode: (document.getElementById('hcode').value || '').toUpperCase().trim(),
            hname: (document.getElementById('hname').value || '').toUpperCase().trim(),
            ...
            expand: document.getElementById('chkExpand').checked ? 'Y' : 'N'
        };
    }

    function applyHead(item) {
        document.getElementById('hcode').value = item.hcode || item.code || '';
        document.getElementById('hname').value = item.hname || item.name || '';
        ...
        document.getElementById('chkExpand').checked = String(item.expand || 'N').toUpperCase() === 'Y';
    }
    ...
    async function loadList() {
        try {
            ...
            (d.data || []).forEach(r => {
                ...
                tr.dataset.code = r.hcode || r.code || '';
                tr.innerHTML = `<td>${escapeHtml(r.hcode || r.code || '')}</td><td>${escapeHtml(r.hname || r.name || '')}</td><td>${escapeHtml(r.actype || '')}</td><td>${escapeHtml(r.pos || '')}</td><td>${String(r.expand || 'N').toUpperCase() === 'Y' ? 'Yes' : 'No'}</td>`;
                body.appendChild(tr);
            });
        } catch (e) { status(e.message, false); }
    }

    async function loadHead(code) {
        try {
            const d = await api({ action: 'load', hcode: code });
            if (d.data) {
                applyHead(d.data);
                if (mode === 'E' || mode === 'D') document.getElementById('hcode').disabled = true;
                status('BS Head loaded: ' + code, true);
            } else {
                status('BS Head not found', false);
            }
        } catch (e) { status(e.message, false); }
    }
    ...
    async function doSave() {
        ...
        if (!p.hcode) { status('Head code is required', false); return; }
        if (!p.hname && mode !== 'D') { status('Head name is required', false); return; }

        if (mode === 'D') {
            if (!confirm('Are you sure you want to delete BS Head ' + p.hcode + '?')) return;
            try {
                const d = await api({ action: 'delete', hcode: p.hcode });
                status(d.message || 'BS Head deleted', true);
        ...
        try {
            if (mode === 'A') {
                const v = await api({ action: 'validate', hcode: p.hcode });
                if (v.exists) { status('BS Head code already exists', false); return; }
            }
            const d = await api({ action: 'save', mode: mode, ...p });
            status(d.message || 'BS Head saved', true);
            resetForm(); setMode('A'); await loadList();
        } catch (e) { status(e.message, false); }
    }
    ...
    document.getElementById('listBody').addEventListener('dblclick', (e) => {
        ...
        if (!tr) return;
        if (mode === 'A') setMode('E');
        loadHead(tr.dataset.code);
    });

    // Mode buttons
    document.getElementById('btnAdd').addEventListener('click', () => { resetForm(); setMode('A'); document.getElementById('hcode').focus(); });
    document.getElementById('btnEdit').addEventListener('click', () => { resetForm(); setMode('E'); document.getElementById('hcode').focus(); });
    document.getElementById('btnDelete').addEventListener('click', () => { resetForm(); setMode('D'); document.getElementById('hcode').focus(); });
    ...
    // Uppercase
    document.getElementById('hcode').addEventListener('input', () => {
        document.getElementById('hcode').value = document.getElementById('hcode').value.toUpperCase();
    });

    // Enter on hcode loads
    document.getElementById('hcode').addEventListener('keydown', (e) => {
        if (e.key === 'Enter') { e.preventDefault(); loadHead(document.getElementById('hcode').value.trim().toUpperCase()); }
    });
