...
<head>
    ...
    <title>Opening Stock Value Set</title>
    <style>
        ...
        .hint { font-size: 11px; color: #627389; margin-top: 2px; }
    </style>
</head>
<body>
<div class="wrap">
    <h1>Opening Stock Value Set</h1>
    ...
    <form method="POST" action="{{ url()->current() }}">
        ...
        <div class="sec">
            <h3>Opening Stock Value</h3>
            <div class="grid">
                <label for="stock_value">Stock Value</label>
                <div>
                    <input id="stock_value" name="stock_value" type="number" step="0.01" value="{{ old('stock_value', $dval ?? 0) }}">
                </div>

                <label for="dtdate">Date</label>
                <div>
                    <input id="dtdate" name="dtdate" type="text" maxlength="10" placeholder="dd/mm/yyyy" value="{{ old('dtdate', $dtdate ?? '') }}">
...
(() => {
    ...
    // Date input formatting: auto-insert slashes
    const dtInput = document.getElementById('dtdate');
    dtInput.addEventListener('input', () => {
        let v = dtInput.value.replace(/[^0-9/]/g, '');
        // Auto-insert slashes
    ...
    dtInput.addEventListener('blur', () => {
        ...
        if (v && !/^\d{2}\/\d{2}\/\d{4}$/.test(v)) {
            dtInput.style.borderColor = '#e74c3c';
        } else {
            dtInput.style.borderColor = '#bfd0e6';
        }
    });
