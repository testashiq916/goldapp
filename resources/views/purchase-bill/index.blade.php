...
const show2 = n => (+n||0) === 0 ? '' : fmt2(n);
const show3 = n => (+n||0) === 0 ? '' : fmt3(n);
const showN = n => (+n||0) === 0 ? '' : String(+n||0);
...
function rfRow(i) {
  ...
  tr.cells[11].querySelector('input').value = show3(r.lesswgt);
  tr.cells[12].querySelector('input').value = show3(r.netwgt);
  tr.cells[14].querySelector('input').value = show2(r.amount);
}
...
function updateFoot() {
  ...
  sv('ftQty',showN(qty)); sv('ftWgt',show3(wgt)); sv('ftStwgt',show3(stwgt));
  sv('ftStprice',show2(stprice)); sv('ftMud',show2(mud));
  sv('ftLesswgt',show3(lesswgt)); sv('ftNetwgt',show3(netwgt));
  sv('ftMc',show2(mc)); sv('ftAmt',show2(amt));
  sv('fBillTotal', show2(amt));
...
function rfExchRow(i) {
  ...
  tr.cells[5].querySelector('input').value = show3(r.lesswgt);
  tr.cells[8].querySelector('input').value = show2(r.amount);
}
function updateExchFoot() {
  ...
  sv('fExchAmtDisplay', show2(amt));          // exchange bar display
  sv('fExchAmtBot', show2(amt));             // bottom section display
  if (!loadingBill) triggerRecalc();
}
function addExchRow() {
...
function doRecalc() {
  ...
  fetch(url('/api/purchase-bill/recalc'),{
    ...
    if(!d||d.error) return;
    sv('fTaxAmt',  show2(d.tax_amt||0));
    sv('fCess',    show2(d.cess||0));
    sv('fTcsAmt',  show2(d.tcs_amt||0));
    sv('fNetTotal',show2(d.net_total||0));
    if(document.activeElement?.id!=='fPaidAmt' && !paidAmtDirty && d.paid_amt!==undefined) {
      sv('fPaidAmt', show2(d.paid_amt||0));
    }
    sv('fBalance', show2(d.balance||0));
    sv('fCB',      show2(d.cb||0));
    if(d.discount!==undefined) sv('fDiscount',show2(d.discount));
    if(d.round!==undefined) sv('fRound',show2(d.round));
  }).catch(()=>{});
}
...
function itemLookup(i){
  ...
  fetch(`${url('/api/purchase-bill/item-lookup')}?code=${encodeURIComponent(code)}&gold_rate=${rate}`)
    .then(r=>r.json()).then(d=>{
      ...
      // After item code lookup, continue entry from Rate with the value selected.
      try {
        const rateInput = $('itbody').rows[i].cells[3].querySelector('input');
        rateInput.focus();
        rateInput.select();
      } catch(e){}
    }).catch(()=>{});
...
function resetForm(){
  ...
  sv('fDate', todayDMY()); sv('fOB',''); sv('fCB','');
  ['fBillTotal','fExchAmtBot','fDiscount','fRound','fTaxAmt','fCess','fHmc','fPReturn','fTcsAmt',
   'fOthers','fPaidAmt','fNetTotal','fBalance','fChqAmt'].forEach(id=>sv(id,''));
  paidAmtDirty = false;
  ['fDiscPerc','fTaxPerc'].forEach(id=>sv(id,''));
  sv('fTcsPerc',''); sv('fChqNo',''); sv('fChqDate',''); sv('fDueDate','');
