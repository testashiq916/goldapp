...
<div class="empty" id="emptyState">Set filters and click <strong>Show</strong> to view the purchase book.</div>
</div>
<script>
const API=@json(url('/api/other-item-reports/purchase-book/data'));
...
document.getElementById('filterForm').addEventListener('submit',async e=>{
  ...
  try{
    ...
    document.getElementById('reportBody').innerHTML=rows.map((r,i)=>`<tr>
      ...
      <td class="num ${r.balance>0?'':''}"> ${n2(r.balance)}</td>
    </tr>`).join('');
