...
<form class="toolbar" id="filterForm">
  ...
  <div class="field"><label>Item</label>
    <select id="icode"><option value="">All Items</option></select>
  </div>
...
<div class="table-wrap" id="tableWrap" style="display:none">
  <table>
    <thead>
      <tr>
        <th>Date</th><th>Doc No</th><th style="text-align:left">Supplier</th>
        <th style="text-align:left">Item</th>
        <th class="num">Qty</th><th class="num">Rate</th><th class="num">Amount</th><th class="num">Cost</th>
      </tr>
    </thead>
    ...
    <tfoot>
      <tr>
        ...
        <td class="num">—</td>
        <td class="num" id="totAmount">0.00</td>
        <td class="num" id="totCost">0.00</td>
      </tr>
    </tfoot>
  </table>
</div>
<div class="empty" id="emptyState">Set filters and click <strong>Show</strong> to view the purchase register.</div>
</div>
<script>
const API_DATA=@json(url('/api/other-item-reports/purchase-register/data'));
...
fetch(API_ITEMS,{headers:{Accept:'application/json'}}).then(r=>r.json()).then(d=>{
  ...
  (d.items||[]).forEach(i=>{
    ...
    o.value=i.code;o.textContent=`${i.name} (${i.code})`;
    sel.appendChild(o);
  });
});
...
document.getElementById('filterForm').addEventListener('submit',async e=>{
  ...
  try{
    ...
    document.getElementById('reportBody').innerHTML=rows.map(r=>`<tr>
      ...
      <td>${esc(r.iname)}<br><small style="color:#5b7088">${esc(r.icode)}</small></td>
      <td class="num">${Number(r.qty||0).toFixed(0)}</td>
      <td class="num">${n2(r.rate)}</td>
      <td class="num">${n2(r.amount)}</td>
      <td class="num">${n2(r.cost)}</td>
    </tr>`).join('');
    document.getElementById('totQty').textContent=Number(t.qty||0).toFixed(0);
    document.getElementById('totAmount').textContent=n2(t.amount);
    document.getElementById('totCost').textContent='—';
    document.getElementById('metaInfo').textContent=`${rows.length} line(s)`;
