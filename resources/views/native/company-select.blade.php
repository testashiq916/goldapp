                        ...
                        <div class="card-actions">
                            ...
                            <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end">
                                ...
                                <button
                                    type="button"
                                    class="switch-btn"
                                    data-company-switch='@json(['database' => $company['database'], 'company_name' => $company['company_name'] !== '' ? $company['company_name'] : $company['database']])'
                                >
                                    Switch Company
                                </button>
                            </div>
    ...
    <div class="switch-login-shell" onclick="event.stopPropagation()">
        <div class="switch-login-card">
            ...
            <div class="switch-login-body">
                ...
                <form method="POST" action="{{ url('/company-select/switch') }}" id="switchCompanyForm">
                    @csrf
...
function submitSwitchCompanyForm() {
    var form = document.getElementById('switchCompanyForm');
    if (!form) {
        return true;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    var switchForm = document.getElementById('switchCompanyForm');
    if (switchForm) {
        switchForm.addEventListener('submit', function() {
            submitSwitchCompanyForm();
        });
    }

    document.querySelectorAll('[data-company-switch]').forEach(function(button) {
        button.addEventListener('click', function() {
            try {
                var payload = JSON.parse(button.getAttribute('data-company-switch') || '{}');
                openSwitchModal(payload.database || '', payload.company_name || payload.database || '');
            } catch (_) {
            }
        });
    });
