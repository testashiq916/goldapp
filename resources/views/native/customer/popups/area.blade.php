...
<head>
    ...
    <title>Clients Area Details</title>
    <style>
...
<body>
<div class="wrap">
    <h2>Clients Area Details</h2>
    @if($missing)<div class="warn">`area` table not found.</div>@endif
    <table>
...
(() => {
    ...
    function render(){
        ...
        rows.forEach((r, i) => {
            ...
            tr.innerHTML = `<td><input data-i="${i}" data-k="code" class="upper" value="${(r.code||'').replace(/"/g,'&quot;')}" maxlength="10"></td>
                            <td><input data-i="${i}" data-k="name" value="${(r.name||'').replace(/"/g,'&quot;')}" maxlength="30"></td>
    ...
    document.getElementById('btnSave').addEventListener('click', async () => {
        ...
        const json = await post(@json(url('/customer/popup/area/save')), { rows: payloadRows, deleted });
    ...
    async function load(){
        deleted.length = 0;
        const json = await post(@json(url('/customer/popup/area/retrieve')), {});
