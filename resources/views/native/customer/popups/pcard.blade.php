...
<head>
    ...
    <title>Clients PCard Details</title>
    <style>
...
<body>
<div class="wrap">
    <h2>Clients PCard Details</h2>
    @if($missing)<div class="warn">`pcard` table not found.</div>@endif
    <table>
        <thead><tr><th style="width:140px">Code</th><th>Name</th><th style="width:100px">VADisc%</th><th style="width:100px">NetAmtD%</th><th style="width:48px"></th></tr></thead>
        <tbody id="tb"></tbody>
    </table>
    <div class="btnbar">
...
(() => {
    ...
    function render(){
        tb.innerHTML = '';
        if (!rows.length) {
            tb.innerHTML = '<tr><td colspan="5" style="text-align:center;color:#777;padding:18px;">No rows.</td></tr>';
            return;
        }
        rows.forEach((r, i) => {
            const tr = document.createElement('tr');
            tr.innerHTML = `<td><input data-i="${i}" data-k="code" class="upper" value="${(r.code||'').replace(/"/g,'&quot;')}" maxlength="10"></td>
                            <td><input data-i="${i}" data-k="name" class="upper" value="${(r.name||'').replace(/"/g,'&quot;')}" maxlength="30"></td>
                            <td><input data-i="${i}" data-k="vadisc" class="num" type="number" step="0.01" value="${Number(r.vadisc||0)}"></td>
                            <td><input data-i="${i}" data-k="totdisc" class="num" type="number" step="0.01" value="${Number(r.totdisc||0)}"></td>
    ...
    tb.addEventListener('input', (e) => {
        ...
        rows[i][k] = (k === 'vadisc' || k === 'totdisc') ? (parseFloat(e.target.value) || 0) : e.target.value;
    });
    ...
    document.getElementById('btnAdd').addEventListener('click', () => {
        ...
        rows.push({ code:'', name:'', vadisc:0, totdisc:0, _new:true }); render();
    });
    document.getElementById('btnSave').addEventListener('click', async () => {
        const payloadRows = rows.map(x => ({
            code:(x.code||'').trim().toUpperCase(),
            name:(x.name||'').trim().toUpperCase(),
            vadisc: Number(x.vadisc||0),
            totdisc: Number(x.totdisc||0)
        })).filter(x => x.code !== '');
        const json = await post(@json(url('/customer/popup/pcard/save')), { rows: payloadRows, deleted });
    ...
    async function load(){
        deleted.length = 0;
        const json = await post(@json(url('/customer/popup/pcard/retrieve')), {});
        if (!json.success) { alert(json.message || 'Load failed'); return; }
        rows.length = 0;
        (json.data || []).forEach(r => rows.push({ code:r.code || '', name:r.name || '', vadisc:Number(r.vadisc||0), totdisc:Number(r.totdisc||0), _new:false }));
        render();
    }
    load();
})();
