@php
    ...
    $codeValue = trim((string)($data['code'] ?? ''));
...
<script>
    ...
    const isPopup = @json((string) request()->query('popup', '') === '1');
    ...
    function loadLatestCustomerCode(force = false) {
        if (!canAutoLoadLatestCode()) return;
        ...
        fetch(siteUrl + '/api/customer/next-code?type=' + encodeURIComponent(type))
            .then(r => r.json())
            .then(out => {
                if (out.success && out.code && codeEl) {
                    lastLoadedCustomerCode = String(out.code).trim();
                    codeEl.value = lastLoadedCustomerCode;
                    customerCodeTouched = false;
                }
            })
            .catch(() => {});
    }

    loadLatestCustomerCode(true);
    ...
    function cancelForm() {
        ...
        setTimeout(function() {
            if (!window.closed) {
                location.href = isPopup
                    ? (siteUrl + '/customer?type=' + encodeURIComponent(type))
                    : (siteUrl + '/dashboard');
            }
        }, 100);
    }
        ...
        fetch(siteUrl + '/api/customer/get?code=' + encodeURIComponent(code))
            ...
            .then(out => {
                if (out.success && out.data) {
                    if (!customerCodeTouched && code === lastLoadedCustomerCode) {
                        loadLatestCustomerCode(true);
                        return;
                    }
    ...
    document.getElementById('btnSave').addEventListener('click', async () => {
        ...
        const autoGeneratedUntouched = !isEdit
            && canAutoLoadLatestCode()
            && !customerCodeTouched
            && currentCode !== ''
            && currentCode === lastLoadedCustomerCode;

        // Let the server reserve the final live code during save when the
        // displayed code is still the untouched auto-suggestion.
        if (autoGeneratedUntouched) {
            formData.set('code', '');
            formData.set('original_code', '');
        }
    ...
    function openLookupPopup(kind) {
        ...
        const url = map[kind];
        if (!url) return;
        const popup = window.open(url, 'popup_' + kind, popupFeatures(780, 600));
    ...
    function getEnterSequence() {
        const selectors = [
            '#customerForm input:not([type="hidden"]):not([type="file"]):not([disabled])',
            '#customerForm select:not([disabled])',
            '#customerForm textarea:not([disabled])',
            '#customerForm button:not([disabled])',
            '.btnbar .btn:not([disabled])'
        ];

        return Array.from(document.querySelectorAll(selectors.join(','))).filter((el) => {
            if (!el) return false;
            if (el.offsetParent === null) return false;
            if (el.closest('.header-links')) return false;
            return true;
        });
    }
    ...
    document.addEventListener('keydown', function(event) {
        ...
        const typeAttr = String(active.type || '').toLowerCase();
        const isTextInput = tag === 'INPUT' && !['hidden', 'file', 'checkbox', 'radio', 'button', 'submit'].includes(typeAttr);
        const isSelect = tag === 'SELECT';
        const isButton = tag === 'BUTTON' || (tag === 'INPUT' && ['button', 'submit'].includes(typeAttr));

        if (!isTextInput && !isSelect && !isButton) return;
        ...
        const sequence = getEnterSequence();
        const index = sequence.indexOf(active);
        if (index === -1) return;

        const next = sequence[index + 1] || document.getElementById('btnSave');
        if (!next) return;
