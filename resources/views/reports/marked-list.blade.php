...
<div class="toolbar">
  ...
  <div class="tb-row">
    ...
    <div class="f-group">
      ...
      <select id="type" class="tb-select" style="width:130px">
        <option value="">All</option>
    ...
    <div class="sep"></div>
    <div class="f-group">
      <span class="tb-lbl">Marked</span>
      <select id="marked" class="tb-select" style="width:140px">
        <option value="">All</option>
        <option value="Y">Marked Items Only</option>
        <option value="N">Not Marked Items</option>
      </select>
    </div>
    <div class="sep"></div>
    <label class="tb-check"><input type="checkbox" id="cbRmnowise"> Remake No. wise</label>
  </div>
</div>
...
function headers(){
  return [
    ['Date','tdate'],['Bill No','billno'],['Item','displayname'],
    ['Qty','qty',1,0],['Sales Wgt','weight',1,3],['St.Wgt','stonewgt',1,3],
    ['Gross Wgt','grosswgt',1,3],['Amount','amount',1],
    ['SmithMC','tsmithmc',1],['Remake No','rmno'],['Marked?','mark']
  ];
}
...
async function loadLookups(){
  try {
    const r = await fetch(SITE+'/api/marked-list/lookups');
    const d = await r.json();
    if(!d.ok) return;
    const sel = document.getElementById('smcode');
    const first = sel.options[0].outerHTML;
    sel.innerHTML = first + (d.salesmen||[]).map(i=>'<option value="'+esc(i.code)+'">'+esc(i.code)+' - '+esc(i.name)+'</option>').join('');
  } catch(e){}
}
loadLookups();
...
async function loadData(){
  const qs = new URLSearchParams({
    ...
    rmnowise: document.getElementById('cbRmnowise').checked ? 1 : 0,
  });

  let sub = 'From: '+document.getElementById('date1').value+' To: '+document.getElementById('date2').value;
  const mk = document.getElementById('marked');
  if(mk.value) sub += ' | '+mk.selectedOptions[0].text;
  document.getElementById('subHeader').textContent = sub;

  try {
    const r = await fetch(SITE+'/api/marked-list/data?'+qs);
    const d = await r.json();
...
function renderSummary(){
  ...
  document.getElementById('summary').innerHTML = [
    ...
    '<span>Gold Gross: <b>'+nf(t.gold_grosswgt,3)+'</b></span>',
    '<span>Gold Amt: <b>'+nf(t.gold_amount)+'</b></span>',
    '<span>Silver Wgt: <b>'+nf(t.silver_weight,3)+'</b></span>',
    '<span>Silver Gross: <b>'+nf(t.silver_grosswgt,3)+'</b></span>',
    '<span>Silver Amt: <b>'+nf(t.silver_amount)+'</b></span>',
    '<span>St.Wgt: <b>'+nf(t.total_stonewgt,3)+'</b></span>',
    '<span>SmithMC: <b>'+nf(t.total_tsmithmc)+'</b></span>',
...
ReportExport.init('btnSaveAs', headers, ()=>rows,
  ()=>'marked-list-'+document.getElementById('date1').value+'-to-'+document.getElementById('date2').value);
</script>
</body>
</html>
