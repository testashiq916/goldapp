...
<div class="toolbar">
  ...
  <div class="f-group"><span class="tb-lbl">Sort</span>
    <select id="selSort" class="tb-select" style="width:120px">
      ...
      <option value="issuewgt">Issued Wgt</option>
...
function ni(n){ return String(n||0); }
...
const cols = [
  { key:'name',     label:'Smith/Jewl',  cls:'',    fmt:'s' },
  { key:'lotno',    label:'Lot No',      cls:'',    fmt:'s' },
  ...
  { key:'issueqty', label:'Issued Qty',  cls:'num', fmt:'i' },
  { key:'rcvdqty',  label:'Rcvd Qty',    cls:'num', fmt:'i' },
  { key:'pendqty',  label:'Pend Qty',    cls:'num', fmt:'i' },
];
...
function fmtCell(r, c){
  const v = r[c.key];
  if(c.fmt==='3') return nf3(v);
  if(c.fmt==='i') return ni(v);
  return esc(v);
}
...
function renderBody(){
  ...
  rows.forEach(r=>{
    html+='<tr>';
    cols.forEach(c=>{
      const cls = c.cls==='num' ? ' class="num"' : '';
      html+='<td'+cls+'>'+fmtCell(r,c)+'</td>';
    });
    html+='</tr>';
  });
...
function renderSummary(){
  ...
  html += '<span>Issued Qty: <b>'+ni(t.issueqty)+'</b></span>';
  html += '<span>Rcvd Qty: <b>'+ni(t.rcvdqty)+'</b></span>';
  html += '<span>Pend Qty: <b>'+ni(t.pendqty)+'</b></span>';
...
function loadData(){
  ...
  const params = new URLSearchParams({date1:d1, date2:d2, rlevel:RLEVEL, smith});
  ...
  fetch(SITE+'/api/smith-lot-report/data?'+params.toString())
    .then(r=>r.json())
    .then(d=>{
      ...
      document.getElementById('subHeader').textContent =
        rows.length+' rows | Period: '+d1+' to '+d2+(smith?' | Smith: '+smith:'');
    })
...
function saveAsCsv(){
  ...
  a.download = 'smith_lot_report_'+document.getElementById('date1').value+'_'+document.getElementById('date2').value+'.csv';
  a.click();
}
