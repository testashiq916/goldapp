...
<div class="toolbar">
  <div class="tb-row">
    ...
    <div class="f-group">
      ...
      <select id="reptype" class="tb-select" style="width:130px">
        ...
        <option value="cosummary">C/o Summary</option>
        <option value="agentsummary">Agent Summary</option>
      </select>
    </div>
    <div class="sep"></div>
  ...
  <div class="tb-row">
    <div class="f-group">
      ...
      <select id="smcode" class="tb-select" style="width:130px">
        <option value="">-- All --</option>
      </select>
    </div>
    <div class="sep"></div>
    <div class="f-group">
      <label class="tb-check"><input type="checkbox" id="cbCounter"> Counter</label>
      <select id="counter" class="tb-select" style="width:120px" disabled>
        <option value="">-- All --</option>
      </select>
    </div>
    <div class="sep"></div>
    <div class="f-group">
      <label class="tb-check"><input type="checkbox" id="cbIc"> IC</label>
      <select id="ic" class="tb-select" style="width:120px" disabled>
        <option value="">-- All --</option>
      </select>
    </div>
    <div class="sep"></div>
    <div class="f-group">
      <span class="tb-lbl">C/o</span>
      <select id="cocode" class="tb-select" style="width:140px">
        <option value="">-- All --</option>
      </select>
    </div>
    <div class="sep"></div>
    <div class="f-group">
      <label class="tb-check"><input type="checkbox" id="cbAgent"> Agent</label>
      <select id="agcode" class="tb-select" style="width:130px" disabled>
        <option value="">-- All --</option>
      </select>
    </div>
  </div>
...
function headers(){
  ...
  if(rt === 'cosummary'){
    return [
      ['C/o Code','cocode'],['C/o Name','coname'],
      ['Bills','billcount',1,0],['Bill Amt','billamt',1],
      ['Discount','discount',1],['Ret Amt','sretamt',1],
      ['Net Amt','netamt',1]
    ];
  }
  if(rt === 'agentsummary'){
    return [
      ['Agent Code','agcode'],['Agent Name','agname'],
      ['Bills','billcount',1,0],['Bill Amt','billamt',1],
      ['Discount','discount',1],['Ret Amt','sretamt',1],
      ['Net Amt','netamt',1]
    ];
  ...
  return [
    ['Date','tdate'],['Bill No','billno'],['Customer','custname'],['SM','smcode'],
    ['S.Gr.Wgt','tsaleswgt',1,3],['S.Net.Wgt','netsalewgt',1,3],
    ['Ret.Wgt','tsretwgt',1,3],['Net S.Wgt','netswgt',1,3],
    ['Sales Amt','billamt',1],['Ret Amt','sretamt',1],
    ['MC Amt','tva',1],['MC%','vap',1],['Disc','discount',1],
    ['Net Amt','netsamt',1]
  ];
}
...
async function loadLookups(){
  try {
    const r = await fetch(SITE+'/api/net-sales/lookups');
    ...
    fillSelect('cocode', d.coaccounts||[]);
  } catch(e){}
}
...
/* ── Checkbox toggles ─────────────────────────────────── */
document.getElementById('cbCounter').addEventListener('change', function(){ document.getElementById('counter').disabled = !this.checked; });
document.getElementById('cbIc').addEventListener('change', function(){ document.getElementById('ic').disabled = !this.checked; });
document.getElementById('cbAgent').addEventListener('change', function(){ document.getElementById('agcode').disabled = !this.checked; });
...
async function loadData(){
  ...
  const qs = new URLSearchParams({
    ...
    agcodeon: document.getElementById('cbAgent').checked ? 1 : 0,
  });

  let sub = 'From: '+document.getElementById('date1').value+' To: '+document.getElementById('date2').value;
  const rtLabel = document.getElementById('reptype').selectedOptions[0].text;
  sub += ' | '+rtLabel;
  document.getElementById('subHeader').textContent = sub;

  try {
    const r = await fetch(SITE+'/api/net-sales/data?'+qs);
    const d = await r.json();
...
function renderSummary(){
  ...
  if(rt === 'cosummary' || rt === 'agentsummary'){
    parts.push('<span>Bills: <b>'+nf(t.billcount,0)+'</b></span>');
    parts.push('<span>Bill Amt: <b>'+nf(t.billamt)+'</b></span>');
    parts.push('<span>Discount: <b>'+nf(t.discount)+'</b></span>');
    parts.push('<span>Ret Amt: <b>'+nf(t.sretamt)+'</b></span>');
    parts.push('<span>Net Amt: <b>'+nf(t.netamt)+'</b></span>');
  } else {
    parts.push('<span>S.Gr.Wgt: <b>'+nf(t.tsaleswgt,3)+'</b></span>');
    parts.push('<span>S.Net.Wgt: <b>'+nf(t.netsalewgt,3)+'</b></span>');
    parts.push('<span>Ret.Wgt: <b>'+nf(t.tsretwgt,3)+'</b></span>');
    parts.push('<span>Net S.Wgt: <b>'+nf(t.netswgt,3)+'</b></span>');
    parts.push('<span>Sales Amt: <b>'+nf(t.billamt)+'</b></span>');
    parts.push('<span>MC Amt: <b>'+nf(t.tva)+'</b></span>');
    parts.push('<span>MC%: <b>'+nf(t.vap)+'</b></span>');
    parts.push('<span>Disc: <b>'+nf(t.discount)+'</b></span>');
    parts.push('<span>Net Amt: <b>'+nf(t.netsamt)+'</b></span>');
  }
...
ReportExport.init('btnSaveAs', headers, ()=>rows,
  ()=>'net-sales-'+document.getElementById('date1').value+'-to-'+document.getElementById('date2').value);
</script>
</body>
</html>
