...
function headers(){
  return [
    ...
    ['Dmd Wgt','dmdwgt',1,3],['Dmd Unit','dmdunit'],
    ['Dmd Amt','dmdamt',1],['Total Amt','amount',1]
  ];
}
...
async function loadData(){
  ...
  try {
    const r = await fetch(SITE+'/api/barcode-register/data?'+qs);
...
function render(){
  ...
  document.getElementById('summary').innerHTML = [
    ...
    '<span>Dmd Wgt: <b>'+nf(t.dmdwgt,3)+'</b></span>',
    '<span>Dmd Amt: <b>'+nf(t.dmdamt)+'</b></span>',
    '<span>Total Amt: <b>'+nf(t.amount)+'</b></span>'
  ].join('');
}
...
ReportExport.init('btnSaveAs', headers, ()=>rows,
  ()=>'barcode-register-'+document.getElementById('date1').value+'-to-'+document.getElementById('date2').value);
</script>
</body>
</html>
