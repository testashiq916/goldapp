...
<div class="toolbar">
  <div class="tb-row">
    ...
    <div class="sep"></div>
    <label class="tb-check"><input type="checkbox" id="cbItemwise"> Itemwise</label>
    <div class="sep"></div>
...
function isItemwise(){ return document.getElementById('cbItemwise').checked; }
...
function headers(){
  if(isItemwise()){
    return [
      ...
      ['Gross Wgt','grosswgt',1,3],['St.Price','stoneprice',1],['Wastage','wastage',1,3],
      ['MC','mcharge',1],['Amount','amount',1]
    ];
  }
  return [
    ['Date','tdate'],['Bill No','billno'],['Item','item_name'],
    ['Qty','qty',1,0],['Ret.Wgt','weight',1,3],['Stone Wgt','stonewgt',1,3],
    ['Gross Wgt','grosswgt',1,3],['St.Price','stoneprice',1],['Wastage','wastage',1,3],
    ['MC','mcharge',1],['Amount','amount',1],['SM','smcode']
  ];
}
...
async function loadData(){
  ...
  if(isItemwise()) sub += ' | Itemwise Summary';
  document.getElementById('subHeader').textContent = sub;

  try {
    const r = await fetch(SITE+'/api/sales-return-register/data?'+qs);
...
function render(){
  ...
  parts.push('<span>MC: <b>'+nf(t.mcharge)+'</b></span>');
...
ReportExport.init('btnSaveAs', headers, ()=>rows,
  ()=>'sales-return-register-'+document.getElementById('date1').value+'-to-'+document.getElementById('date2').value);
</script>
</body>
</html>
