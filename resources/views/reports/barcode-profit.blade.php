...
<div class="toolbar">
  ...
  <div class="tb-row">
    <div class="f-group">
      <label class="tb-check"><input type="checkbox" id="cbGrp"> Group</label>
      <select id="grpcode" class="tb-select" style="width:140px" disabled>
        <option value="">-- All --</option>
      </select>
    </div>
    <div class="sep"></div>
    <div class="f-group">
      <label class="tb-check"><input type="checkbox" id="cbSubgrp"> SubGroup</label>
      <select id="subgrp" class="tb-select" style="width:140px" disabled>
        <option value="">-- All --</option>
      </select>
    </div>
  </div>
...
function headers(){
  return [
    ...
    ['Item','itemname'],['Barcode','bcode'],
    ['Qty','qty',1,0],['Gross Wgt','weight',1,3],['St.Wgt','stonewgt',1,3],['Net Wgt','netwgt',1,3],
    ['Sales Amt','amount',1],['Cost Amt','costamt',1],
    ['C%','costperc',1],['Profit','profit',1],
    ['StAmt','stamt',1],['StCost','stcost',1],['StProfit','stprofit',1]
  ];
}
...
async function loadLookups(){
  try {
    const r = await fetch(SITE+'/api/barcode-profit/lookups');
...
document.getElementById('cbGrp').addEventListener('change', function(){ document.getElementById('grpcode').disabled = !this.checked; });
document.getElementById('cbSubgrp').addEventListener('change', function(){ document.getElementById('subgrp').disabled = !this.checked; });
...
async function loadData(){
  const qs = new URLSearchParams({
    ...
    rlevel: RLEVEL,
    grpcode: document.getElementById('cbGrp').checked ? document.getElementById('grpcode').value : '',
    grpon: document.getElementById('cbGrp').checked ? 1 : 0,
    subgrp: document.getElementById('cbSubgrp').checked ? document.getElementById('subgrp').value : '',
    subgrpon: document.getElementById('cbSubgrp').checked ? 1 : 0,
  });
  ...
  try {
    const r = await fetch(SITE+'/api/barcode-profit/data?'+qs);
    const d = await r.json();
...
function render(){
  ...
  document.getElementById('summary').innerHTML = [
    '<span>Count: <b>'+nf(t.count,0)+'</b></span>',
    ...
    '<span>Sales Amt: <b>'+nf(t.amount)+'</b></span>',
    '<span>Cost Amt: <b>'+nf(t.costamt)+'</b></span>',
    '<span>Profit: <b>'+nf(t.profit)+'</b></span>',
    '<span>StAmt: <b>'+nf(t.stamt)+'</b></span>',
    '<span>StCost: <b>'+nf(t.stcost)+'</b></span>',
    '<span>StProfit: <b>'+nf(t.stprofit)+'</b></span>'
  ].join('');
}
...
ReportExport.init('btnSaveAs', headers, ()=>rows,
  ()=>'barcode-profit-'+document.getElementById('date1').value+'-to-'+document.getElementById('date2').value);
</script>
</body>
</html>
