...
<div class="toolbar">
  ...
  <div class="sep"></div>
  <div class="f-group">
    <label class="tb-check"><input type="checkbox" id="cbBtype" checked> BType</label>
    <select id="btype" class="tb-select" style="width:120px"><option value="">-- All --</option></select>
  </div>
  <div class="sep"></div>
...
<div class="grid-wrap">
  <table>
    ...
    <tbody id="tbody"><tr><td colspan="16" style="text-align:center;padding:40px;color:#94a3b8">Press <b>Show</b> to load data</td></tr></tbody>
  </table>
</div>
...
function headers(){
  return [
    ...
    ['Disc','discount',1],['%','discperc',1,1],['Net Amt','netamt',1],
    ['Gross Wgt','grosswgt',1,3],['Net Wgt','netwgt',1,3],['Gold Wgt','totalwgt',1,3],['Total VA','ctotva',1],['VA/Gm','vapergm',1],
    ['VA%','vap',1,1],['D%','discperc2',1,1],
    ['VA AfterDisc','vaafterdisc',1],['VA/Gm*','vaafterdiscpergm',1]
  ];
}
...
async function loadLookups(){
  try {
    const r = await fetch(SITE+'/api/va-check-report/lookups');
    const d = await r.json();
    if(!d.ok) return;
    const sel = document.getElementById('btype');
...
async function loadData(){
  const qs = new URLSearchParams({
    ...
    rlevel: RLEVEL,
    btype: document.getElementById('cbBtype').checked ? document.getElementById('btype').value : '',
    btypeon: document.getElementById('cbBtype').checked ? 1 : 0,
  });
  ...
  try {
    const r = await fetch(SITE+'/api/va-check-report/data?'+qs);
    const d = await r.json();
...
function render(){
  ...
  document.getElementById('summary').innerHTML = [
    ...
    '<span>SR Amt: <b>'+nf(t.sretamt)+'</b></span>',
    ...
    '<span>Gold Wgt: <b>'+nf(t.totalwgt,3)+'</b></span>',
    '<span>VA: <b>'+nf(t.ctotva)+'</b></span>',
    '<span>VA/Gm: <b>'+nf(t.vapergm)+'</b></span>',
    '<span>VA AfterDisc: <b>'+nf(t.vaafterdisc)+'</b></span>',
    '<span>VA/Gm*: <b>'+nf(t.vaafterdiscpergm)+'</b></span>',
  ].join('');
}
...
ReportExport.init('btnSaveAs', headers, ()=>rows,
  ()=>'va-check-report-'+document.getElementById('date1').value+'-to-'+document.getElementById('date2').value);
</script>
</body>
</html>
