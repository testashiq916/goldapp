...
<style>
  ...
  .meta-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-top:10px;font-size:12px}
...
<div class="page" data-print-root="order-slip">
  ...
  <div class="meta-grid">
    <div class="box tight">
      <div class="meta-row"><div class="lbl">Cash Paid</div><div>:</div><div>{{ $fmtAmt($cashPaid) }}</div></div>
    </div>
    <div class="box tight">
    ...
    <div class="box tight">
      <div class="meta-row"><div class="lbl">Payment Mode</div><div>:</div><div>{{ $cashBankLabel !== '' ? $cashBankLabel : 'CASH' }}</div></div>
      @if($showChequeAmount)
        <div class="meta-row"><div class="lbl">Bank Amount</div><div>:</div><div>{{ $fmtAmt($order['chq_amt'] ?? 0) }}{{ $chqBankLabel !== '' ? ' - ' . $chqBankLabel : '' }}</div></div>
      @endif
    </div>
  </div>
