...
<div class="table-wrap" id="tableWrap" style="display:none">
  <table>
    <thead>
      <tr>
        <th style="text-align:left">Refiner</th>
        <th class="num">Fwd Bills</th>
        <th class="num">Ret Bills</th>
        <th class="num">Total Issued (g)</th>
        <th class="num">Total Received (g)</th>
        <th class="num">Difference (g)</th>
        <th class="num">Mud Less (g)</th>
        <th class="num">Test Pcs (g)</th>
      </tr>
    </thead>
    ...
    <tfoot>
      <tr>
        <td>Total</td>
        <td class="num" id="totFwdBills">0</td>
        <td class="num" id="totRetBills">0</td>
...
<div class="empty" id="emptyState">Set filters and click <strong>Show</strong> to view the refiner's summary.</div>
</div>
<script>
const API = @json(url('/api/refinery-reports/refiners-summary/data'));
...
document.getElementById('filterForm').addEventListener('submit', async e=>{
  ...
  try{
    ...
    document.getElementById('reportBody').innerHTML=rows.map(r=>`<tr>
      <td><strong>${esc(r.refinername)}</strong><br><small style="color:#5b7088">${esc(r.refcode)}</small></td>
      <td class="num">${r.fwd_bills}</td>
      <td class="num">${r.ret_bills}</td>
      <td class="num pos">${n3(r.total_issued)}</td>
      <td class="num pos">${n3(r.total_rcvd)}</td>
      <td class="num ${r.difference>=0?'pos':'neg'}">${n3(r.difference)}</td>
      <td class="num">${n3(r.total_mudless)}</td>
      <td class="num">${n3(r.total_testpcs)}</td>
    </tr>`).join('');
    document.getElementById('totFwdBills').textContent=rows.reduce((s,r)=>s+r.fwd_bills,0);
    document.getElementById('totRetBills').textContent=rows.reduce((s,r)=>s+r.ret_bills,0);
    document.getElementById('totIssued').textContent=n3(t.total_issued);
    document.getElementById('totRcvd').textContent=n3(t.total_rcvd);
    document.getElementById('totDiff').textContent=n3(t.difference);
    document.getElementById('totMudless').textContent=n3(t.total_mudless);
    document.getElementById('totTestpcs').textContent=n3(t.total_testpcs);
    document.getElementById('metaInfo').textContent=`${rows.length} refiner(s)`;
