...
.fwd{color:#1b5b31}.ret{color:#7a1200}
  ...
  <table>
    <thead>
      <tr>
        ...
        <th>Refiner</th>
        <th>Item</th>
        <th class="num">Issued Wgt</th>
        <th class="num">Rcvd Wgt</th>
        <th class="num">Issued Touch</th>
        <th class="num">Rcvd Touch</th>
        <th class="num">Mud Less</th>
        <th class="num">Test Pcs</th>
...
<div class="empty" id="emptyState">Set filters and click <strong>Show</strong> to view the entry report.</div>
</div>
<script>
const API = @json(url('/api/refinery-reports/entry-report/data'));
...
document.getElementById('filterForm').addEventListener('submit', async e=>{
  ...
  try{
    ...
    document.getElementById('reportBody').innerHTML=rows.map(r=>`<tr>
      ...
      <td class="num fwd">${r.issuedwgt>0?n3(r.issuedwgt):''}</td>
      <td class="num ret">${r.rcvdwgt>0?n3(r.rcvdwgt):''}</td>
