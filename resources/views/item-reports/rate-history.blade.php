...
input{height:32px;border:1px solid #bfd0e6;border-radius:5px;padding:0 8px;font-size:18px;box-sizing:border-box;width:140px;color:#000080;font-weight:700}
...
tr.day-row td{background:#fffaee;font-weight:600}
.empty{padding:30px;text-align:center;color:#64748b;border:1px dashed #c7d4e4;border-radius:8px;margin-top:8px}
@media print{.toolbar{display:none}body{background:#fff}.wrap{border:0;margin:0}.table-wrap{max-height:none;overflow:visible;border:0}th{position:static}}
</style>
...
<form class="toolbar" id="filterForm">
    <div class="field">
        <label>From</label>
        <input type="date" id="date1" value="{{ $today }}">
    </div>
    <div class="field">
        <label>To</label>
        <input type="date" id="date2" value="{{ $today }}">
    </div>
...
<div class="table-wrap" id="tableWrap" style="display:none">
    <table>
        <thead><tr>
            ...
            <th>Gold Rate</th><th>Silver Rate</th><th>Thangam Rate</th>
            <th>Platinum</th><th>Ounce Rate</th><th>Conversion</th><th>Bul.Rate</th><th>Bul.Touch</th>
        </tr></thead>
...
<script>
const API=@json(url('/api/rate-history/data'));
...
document.getElementById('filterForm').addEventListener('submit',async e=>{
    ...
    const p=new URLSearchParams({date1:document.getElementById('date1').value,date2:document.getElementById('date2').value});
    try{
        ...
        document.getElementById('reportBody').innerHTML=rows.map(r=>{
            const cls=r.tdate!==lastDate?'day-row':'';lastDate=r.tdate;
            return`<tr class="${cls}">
            <td>${fmtDate(r.tdate)}</td><td>${fmtTime(r.ttime)}</td>
            <td class="num">${fmtN(r.grate,2)}</td><td class="num">${fmtN(r.srate,2)}</td><td class="num">${fmtN(r.thrate,2)}</td>
            <td class="num">${fmtN(r.prate,2)}</td><td class="num">${fmtN(r.ouncerate,4)}</td><td class="num">${fmtN(r.conversion,4)}</td>
            <td class="num">${fmtN(r.bulrate,2)}</td><td class="num">${fmtN(r.bultouch,2)}</td>
            </tr>`;
        }).join('');
        ...
        if(!rows.length)document.getElementById('emptyState').textContent='No rate history found for the selected period.';
    }catch(err){
