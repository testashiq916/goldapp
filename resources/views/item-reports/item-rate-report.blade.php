...
input,select{height:32px;border:1px solid #bfd0e6;border-radius:5px;padding:0 8px;font-size:12px;box-sizing:border-box}
...
<form class="toolbar" id="filterForm">
    ...
    <div class="field" style="justify-content:flex-end"><button type="submit" class="primary">Show</button></div>
    <div class="field" style="justify-content:flex-end"><button type="button" onclick="window.print()">Print</button></div>
</form>
...
<div class="table-wrap" id="tableWrap" style="display:none">
    <table>
        <thead><tr>
            ...
            <th>Touch</th><th>Wastage</th><th>MC</th>
            <th>Rate</th><th>WS Rate</th><th>Cost</th>
            <th>P.Rate</th><th>C.Rate</th><th>Def.StkT</th>
        </tr></thead>
...
<script>
const API=@json(url('/api/item-rate-report/data'));
...
document.getElementById('filterForm').addEventListener('submit',async e=>{
    ...
    const p=new URLSearchParams({itype:document.getElementById('itype').value,search:document.getElementById('search').value.trim()});
    try{
        ...
        document.getElementById('reportBody').innerHTML=rows.map(r=>`<tr>
            ...
            <td class="num">${fmtN(r.touch,3)}</td><td class="num">${fmtN(r.wastage,3)}</td><td class="num">${fmtN(r.mcharge,2)}</td>
            <td class="num">${fmtN(r.rate,2)}</td><td class="num">${fmtN(r.wsrate,2)}</td><td class="num">${fmtN(r.cost,2)}</td>
            <td class="num">${fmtN(r.prate,2)}</td><td class="num">${fmtN(r.crate,2)}</td><td style="text-align:center">${esc(r.defstktype)}</td>
        </tr>`).join('');
