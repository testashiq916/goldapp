...
td.num{text-align:right;font-family:'Courier New',monospace;white-space:nowrap;color:#7b3fb5}
...
<div class="table-wrap" id="tableWrap" style="display:none">
    <table>
        <thead><tr>
            <th style="text-align:left">Code</th><th style="text-align:left">Item Name</th>
            <th>Qty Sold</th><th>Stone Wgt</th><th>Stone Price</th>
        </tr></thead>
        <tbody id="reportBody"></tbody>
        <tfoot><tr>
            <td colspan="2">Total</td>
            <td class="num" id="totQty">0</td>
            <td class="num" id="totStonewgt">0.000</td>
            <td class="num" id="totStoneprice">0.00</td>
        </tr></tfoot>
    </table>
</div>
...
<script>
const API=@json(url('/api/stone-trans-analysis/data'));
...
document.getElementById('filterForm').addEventListener('submit',async e=>{
    ...
    try{
        ...
        document.getElementById('reportBody').innerHTML=rows.map(r=>`<tr>
            <td>${esc(r.code)}</td><td>${esc(r.iname||r.code)}</td>
            <td class="num">${fmtN0(r.tot_qty)}</td>
            <td class="num">${fmtN(r.tot_stonewgt,3)}</td>
            <td class="num">${fmtN(r.tot_stoneprice,2)}</td>
        </tr>`).join('');
        document.getElementById('totQty').textContent=fmtN0(t.qty)||'0';
        document.getElementById('totStonewgt').textContent=(t.stonewgt||0).toFixed(3);
        document.getElementById('totStoneprice').textContent=(t.stoneprice||0).toFixed(2);
        ...
        if(!rows.length)document.getElementById('emptyState').textContent='No stone transactions found for the selected period.';
    }catch(err){
