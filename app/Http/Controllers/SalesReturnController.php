...
class SalesReturnController extends Controller
{
    ...
    public function customers(Request $request)
    {
        ...
        $selects = ['code', 'name'];
        $mobileColumns = ['mobile', 'mobno', 'telephone', 'phone'];
        $mobileColumn = null;
        foreach ($mobileColumns as $candidate) {
        ...
        if ($mobileColumn !== null) {
            $selects[] = DB::raw("TRIM(COALESCE($mobileColumn, '')) AS mobile");
        } else {
            $selects[] = DB::raw("'' AS mobile");
        }

        $q = DB::table($table)->orderBy('name')->limit(800);
        if (Schema::hasColumn($table, 'disabled')) $q->where('disabled', 0);

        return response()->json(['success' => true, 'data' => $q->get($selects)]);
    }
    ...
    public function cashBanks(Request $request)
    {
        if (!$this->auth($request)) return $this->unauth();
    ...
    public function get(Request $request)
    {
        ...
        $master->cbcode = trim((string) ($master->cbcode ?? ''));
        if ($master->cbcode === '' && $this->hasTable('daybook')) {
            $cbRow = DB::table('daybook')
                ->where('slno', $slno)
                ->where('amount', '>', 0)
                ->whereNotIn('accode', [$master->custcode ?? '', 'DISC', 'SGST', 'CGST', 'IGST', 'AST', 'CESS'])
                ->orderByDesc('amount')
                ->first(['accode']);
            if ($cbRow) {
                $master->cbcode = trim((string) ($cbRow->accode ?? ''));
            }
        }
        if ($master->cbcode === '') {
            $master->cbcode = 'CASH';
        }
        $creditFlag = strtoupper(trim((string) ($master->credit ?? $master->loan ?? 'N')));
        $master->credit = $creditFlag === 'Y' ? 'Y' : 'N';
    ...
    public function save(Request $request)
    {
        ...
        try {
            ...
            $credit   = strtoupper(trim((string) ($data['credit'] ?? 'N'))) === 'Y' ? 'Y' : 'N';
            $cbcode   = strtoupper(trim((string) ($data['cbcode'] ?? 'CASH')));
            if ($cbcode === '' || $cbcode === 'CASH IN HAND') {
                $cbcode = 'CASH';
            }
            if ($credit === 'Y') {
                $pamt = 0.0;
            ...
            if ($isEdit) {
                ...
                if (Schema::hasColumn('salesrm', 'cbcode')) {
                    $masterRow['cbcode'] = $cbcode;
                }
                if (Schema::hasColumn('salesrm', 'credit')) {
                    $masterRow['credit'] = $credit;
                } elseif (Schema::hasColumn('salesrm', 'loan')) {
                    $masterRow['loan'] = $credit;
                }
                DB::table('salesrm')->where('slno', $slno)->update($masterRow);
            } else {
                ...
                DB::table('salesrm')->insert($masterRow);
            }
            ...
            if ($pamt > 0) {
                DB::table('daybook')->insert(['slno' => $slno, 'accode' => $cbcode,
