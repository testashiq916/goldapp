...
class OrderSaleController extends Controller
{
    ...
    public function save(Request $request): JsonResponse
    {
        ...
        $skipLinkedOrderExchangePosting = empty($exchange)
            && $orderno !== ''
            && $this->hasTable('orderm')
            && DB::table('orderm')->whereRaw('TRIM(ordno)=?', [$orderno])->exists();

        try {
        DB::transaction(function () use ($existingSalesm, $row, $slno, $items, $exchange, $salesReturn, $payload, $calc, $sqlTime, $billNo, $custCode, $custName, $billType, $smCode, $counter, $rate, $cbcode, $addr, $isCst, $control, $billDate, $extra, $taxPerc, $addBc, $isEdit, $orderno, $skipLinkedOrderExchangePosting) {
            ...
            if (!$skipLinkedOrderExchangePosting && $exAmt > 0 && $this->hasTable('purchasem')) {
                $pmRow = [
                    'slno' => $slno,
                    'tdate' => $billDateSql,
                    'ttime' => $sqlTime,
            ...
            if (!$skipLinkedOrderExchangePosting && $exchangeCol->isNotEmpty() && $this->hasTable('purchased')) {
                $sno = 1;
                $rows = [];
            ...
            $this->applyLegacyInventoryEffects(
                $items, $exchange, $salesReturn, $slno, $control, $billDateSql, $skipLinkedOrderExchangePosting
            );
    ...
    private function calcTotals(array $payload): array
    {
        ...
        if ($exchangeAmount <= 0) {
            $exchangeAmount = round($this->toNum($extra['carry_exchange_amount'] ?? 0), 2);
        }
    ...
    private function applyLegacyInventoryEffects(
        array $items,
        array $exchange,
        array $salesReturn,
        int $slno,
        int $control,
        string $billDateSql,
        bool $skipExchangePosting = false
    ): void
    {
