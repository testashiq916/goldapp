...
class AccountMasterController extends Controller
{
    ...
    private function openingBalanceColumn(Request $request): string
    {
        $gilevel = (int) ($request->session()->get('gilevel', 1));
        return $gilevel === 1 ? 'opbal' : 'opbalb';
    }
    ...
    private function saveAccountInternal(Request $request): array
    {
        ...
        $openingValue = $this->normalizeAmount($request->input('balance', 0), strtolower((string) $request->input('balance_type', 'debit')));
        $openingColumn = $this->openingBalanceColumn($request);
        ...
        if ($this->hasColumn('accountm', $openingColumn)) {
            $row[$openingColumn] = $openingValue;
        } else {
            $row['opbal'] = $openingValue;
            $row['opbalb'] = $openingValue;
        }
