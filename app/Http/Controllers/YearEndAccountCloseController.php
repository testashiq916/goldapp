...
class YearEndAccountCloseController extends Controller
{
    ...
    private function resetOpeningStock(): void
    {
        if ($this->hasTable('items')) {
            $update = [];
            foreach (['opweight', 'opweightb', 'opqty', 'opqtyb', 'opstonewgt', 'opstonewgtb', 'opstoneamt', 'opstoneamtb', 'opdmdwgt'] as $column) {
                if ($this->hasCol('items', $column)) {
                    $update[$column] = 0;
                }
            }
            if ($update !== []) {
                DB::table('items')->update($update);
            }
        }
        if ($this->hasTable('itemsstk')) {
            $update = [];
            foreach (['opweight', 'opweightb', 'opqty', 'opqtyb', 'opstonewgt', 'opstonewgtb', 'opstoneamt', 'opstoneamtb', 'opdmdwgt'] as $column) {
                if ($this->hasCol('itemsstk', $column)) {
                    $update[$column] = 0;
                }
            }
            if ($update !== []) {
                DB::table('itemsstk')->update($update);
            }
        }
    }
