...
class PaymentController extends Controller
{
    ...
    private function getAccountBalance(string $accode, int $control): float
    {
        ...
        // Match Cash Book's logic exactly so this figure equals the Cash Book closing balance:
        //   • control <= $control (include lower levels)
        //   • JOIN daybookpart (only entries that have a part row, same as Cash Book)
        //   • date <= today (skip post-dated entries)
    ...
    private function paymentVoucherPrefixes(): array
    {
        $prefixes = ['VP'];

        $secondaryPrefix = trim($this->secondaryPrefixFor('payment'));
        if ($secondaryPrefix !== '') {
    ...
    private function applyPaymentVoucherFilter($query)
    {
        $prefixes = $this->paymentVoucherPrefixes();
    ...
    private function actionAccountList(Request $request): JsonResponse
    {
        ...
        if ($search !== '') {
            $like = '%' . $search . '%';
            // Also match codes whose linked clients row has a matching mobile/telephone/idno/PAN.
            $clientCodes = [];
    ...
    private function actionList(Request $request, int $control): JsonResponse
    {
        ...
        $this->applyPaymentVoucherFilter($query);
        ...
        if ($dir === 'prev') {
            $row = DB::table('daybookpart')
                ->where(function ($inner) {
                    foreach ($this->paymentVoucherPrefixes() as $index => $prefix) {
                        $method = $index === 0 ? 'whereRaw' : 'orWhereRaw';
