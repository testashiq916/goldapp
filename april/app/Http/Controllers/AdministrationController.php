...
class AdministrationController extends Controller
{
        ...
        foreach ($selected as $module) {
            switch ($module) {
                case 'all_transactions':
                    $append(['sales', 'purchase', 'order', 'order_sale', 'sales_return', 'purchase_return', 'receipt', 'payment', 'journal', 'barcode', 'customer_master', 'supplier_master']);
                    break;
                case 'all_masters':
                    $append(['staff_master', 'items_master', 'customer_master', 'supplier_master', 'salesman_master', 'smith_master']);
                    break;
                case 'detail_tables':
                    $append(['sales', 'purchase', 'sales_return', 'purchase_return', 'smith', 'jewellery']);
                    break;
                ...
                case 'daybook_accounts':
                    $append(['receipt', 'payment', 'journal', 'customer_master', 'supplier_master']);
                    break;
                case 'sales':
                case 'order':
                case 'order_sale':
                case 'sales_return':
                    $append([$module, 'customer_master']);
                    break;
                case 'purchase':
                case 'purchase_return':
                    $append([$module, 'supplier_master']);
                    break;
                case 'receipt':
                case 'payment':
                case 'journal':
                    $append([$module, 'customer_master', 'supplier_master']);
                    break;
                default:
    ...
    private function upsertRowsToDatabase(string $database, string $table, array $rows, array $candidateKeys): int
    {
        ...
        foreach ($rows as $row) {
            ...
            DB::table(DB::raw($this->qualifiedTable($database, $table)))->updateOrInsert($where, $filtered);
            $copied++;
        }
