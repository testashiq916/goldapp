...
document.getElementById('closeBtn').addEventListener('click', async()=>{if(!closeDateEl.value){alert('Please choose a closing date.'); return;} if(!confirm('You are going to close old entries up to the selected date. Please make sure reports and backups are already taken. Continue?')) return; statusBox.textContent='Running year-end close... (please wait, this may take a minute)'; try{const json=await postJson('{{ url('/api/year-end-account-close/close') }}',buildPayload()); showStatus((json.message||'Done.') + (json.note ? `\n\nNote\n${json.note}` : ''), json.summary||{});}catch(err){statusBox.textContent='Error: '+(err.message||err);}});
