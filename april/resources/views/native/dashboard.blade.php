    ...
    <div class="content">
        <div class="home-screen" id="homeScreen">
            ...
            <div class="stat-row dash-row">
                <div class="stat-card sc-teal" data-requires-module="sales-bill"><div class="stat-top"><div class="stat-label">Today Sales</div><div class="stat-icon si-teal"><i class="fas fa-cart-shopping"></i></div></div><div class="stat-val">&#8377;{{ number_format($dashboardData['todaySales'] ?? 0, 2) }}</div><div class="stat-foot"><span class="stat-sub">{{ (int)($dashboardData['todaySalesBills'] ?? 0) }} bills today</span><div class="stat-progress"><div class="stat-bar"></div></div></div></div>
...
function openPopupWindow(moduleId, title, cfg) {
    ...
    win.innerHTML =
        ...
        '<div class="window-toolbar">' +
            ...
            '<span class="window-url" title="' + escapeHtml(cfg.url || '') + '">' + escapeHtml(cfg.url || '') + '</span>' +
        '</div>' +
        '<div class="window-content">' +
            (cfg.mode === 'iframe' ? '<iframe class="module-iframe" src="' + escapeHtml(cfg.url) + '"></iframe>' : buildModuleMarkup(moduleId, title)) +
        '</div>';
...
function refreshDashboardRates() {
    return fetch(@json(url('/api/rate/current')) + '?_=' + Date.now(), {
        headers: {
...
function initModuleFrameClose() {
    ...
    window.addEventListener('message', function(event) {
        ...
        if (data.type === 'goldapp:rate-updated') {
            fetch(@json(url('/api/rate/current')))
                .then(function(r){ return r.json(); })
                .then(function(d){
            ...
            if (activeTabId) {
                for (var i = 0; i < openTabs.length; i++) {
                    if (openTabs[i].id === activeTabId) {
                        ...
                        openTabs[i].iframe.src = String(data.url);
                        break;
                    }
                }
            } else {
                // No active tab — open as new tab
                openInDashboardFrameNew(data.moduleId || ('url-' + Date.now()), data.title || 'Module', { mode: 'iframe', url: String(data.url) });
            }
        }
    });
}
