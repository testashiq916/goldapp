...
<title>Supplier Billwise Payment</title>
<style>
...
.hdr-icon{width:30px;height:30px;border-radius:7px;background:linear-gradient(135deg,#2563EB,#1E40AF);display:flex;align-items:center;justify-content:center;}
...
.btn-save{background:linear-gradient(135deg,#2563EB,#1E40AF);color:#fff;box-shadow:0 2px 6px rgba(30,64,175,.3);}
...
.fi input:focus,.fi select:focus{border-color:#2563EB;box-shadow:0 0 0 2px rgba(37,99,235,.15);background:#fff;}
...
.info-lbl{font-size:11.5px;font-weight:700;color:#1E40AF;background:#EFF6FF;border:1px solid #BFDBFE;border-radius:6px;padding:5px 10px;min-width:140px;max-width:260px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.bal-lbl{font-size:12px;font-weight:700;color:#B91C1C;background:#FEF2F2;border:1px solid #FECACA;border-radius:6px;padding:5px 12px;min-width:100px;text-align:right;}
...
.tbl-input:focus{border-color:#2563EB;box-shadow:0 0 0 2px rgba(37,99,235,.12);}
.tbl-date{width:88px;}
.tbl-chk{width:16px;height:16px;accent-color:#2563EB;cursor:pointer;}
...
.cbx-row input[type=checkbox]{accent-color:#2563EB;width:14px;height:14px;}
...
.srch-input:focus{border-color:#2563EB;box-shadow:0 0 0 2px rgba(37,99,235,.12);}
.srch-list{list-style:none;}
.srch-list li{padding:7px 10px;cursor:pointer;border-radius:5px;display:flex;gap:10px;}
.srch-list li:hover,.srch-list li.active{background:#EFF6FF;}
.srch-code{font-weight:700;min-width:70px;color:#1E40AF;}
...
<div class="hdr">
    ...
    <div class="hdr-title">Supplier Billwise Payment</div>
    <div class="vch-badge" id="vchBadge">VPB/-----</div>
</div>
...
<div class="body">
    ...
    <!-- Row 1: Date / GRate / Cash/Bank A/c -->
    <div class="card">
        <div class="card-body">
            <div class="fg">
                ...
                <div class="fi fi-w140">
                    <label>Cash/Bank A/c</label>
                    <input type="text" id="inCbCode" value="CASH" style="text-transform:uppercase" maxlength="20">
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            <div class="fg" style="align-items:center;">
                <div class="fi fi-w110">
                    ...
                    <div class="cust-wrap">
                        <input type="text" id="inSupp" placeholder="Code" style="text-transform:uppercase;"
                               onkeydown="onSuppKey(event)">
                ...
                <div class="fi" style="flex:1">
                    <label>Name</label>
                    <div class="info-lbl" id="suppName">—</div>
                </div>
                <div class="fi">
                    <label>Balance (Payable)</label>
                    <div class="bal-lbl" id="suppBal">—</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <table id="billsTable">
            ...
            <tbody id="billsTbody">
                <tr><td colspan="12" style="text-align:center;padding:20px;color:#9ca3af;">
                    Enter a supplier code to load bills.
                </td></tr>
            </tbody>
            <tfoot id="billsTfoot" style="display:none">
                <tr style="background:#f9fafb;font-weight:700;font-size:12px;">
                    <td colspan="3" style="padding:5px 8px;color:#374151;text-align:right">Total:</td>
                    <td class="r" id="ftNetAmt" style="padding:5px 8px;"></td>
                    <td></td>
                    <td class="r" id="ftPaid" style="padding:5px 8px;"></td>
    ...
    <div class="ftr-strip">
        <div class="ftr-row">
            ...
            <div class="ftr-stat" style="flex:1;min-width:160px">
        ...
        <div class="cbx-row" style="margin-top:8px;">
            <label><input type="checkbox" id="chkPrint"> Print</label>
        </div>
    </div>
    ...
    <div class="modal">
        ...
        <div class="modal-body">
            ...
            <ul class="srch-list" id="modalSrchList">
                <li class="srch-empty">Type to search suppliers…</li>
            </ul>
        </div>
    </div>
</div>
...
<script>
const API = '{{ url("/api/accounts/supplier-billwise-payment") }}';
...
async function loadSupplier(code) {
    ...
    try {
        const d = await post({ action: 'load_supplier', code, grate });
        if (!d.ok) { showToast('error', d.msg || 'Supplier not found'); return; }
        document.getElementById('suppName').textContent = d.name || '—';
        document.getElementById('suppBal').textContent  = fmt2(d.balance || 0);
        allBills = d.bills || [];
        renderBills();
    } catch(e) { showToast('error', 'Load failed'); }
}

function onSuppKey(e) {
    if (e.key === 'Enter') {
        const v = document.getElementById('inSupp').value.trim();
        if (v) loadSupplier(v);
    }
...
function renderBills() {
    ...
    visibleBills = allBills.filter(b => {
        ...
        if (nameFilter && !b.suppname.toLowerCase().startsWith(nameFilter)) return false;
        return true;
    });
    ...
    tbody.innerHTML = visibleBills.map((b, i) => `
        <tr data-idx="${i}">
            ...
            <td class="r"><input type="number" class="tbl-input" id="ai${i}" value="${fmt2(b.alocamt)}" min="0" step="0.01" oninput="onAlocChange(${i},this.value)" onblur="fixAmt(${i},this)"></td>
...
function fixAmt(i, el) {
    const b   = visibleBills[i];
    const val = parseFloat(el.value) || 0;
    if (val > b.balance + 0.005) {
        showToast('error', `Allocated amt (${b.docno || b.billno}) exceeds balance`);
...
function onTotalAllocChange() { updateNonAlloc(); }
...
function updateFooter() {
    let sumNet=0, sumPaid=0, sumBal=0, sumAloc=0, sumDisc=0, sumNB=0;
    ...
    f('ftNetAmt', sumNet); f('ftPaid', sumPaid); f('ftBal', sumBal);
...
async function doSave() {
    const suppcode = document.getElementById('inSupp').value.trim().toUpperCase();
    if (!suppcode) { showToast('error', 'No supplier selected'); return; }
    ...
    const payload = {
        ...
        suppcode,
        cbcode:   document.getElementById('inCbCode').value.trim().toUpperCase() || 'CASH',
...
// ── Clear ─────────────────────────────────────────────────────────────
function doClear() {
    document.getElementById('inSupp').value    = '';
    document.getElementById('inPart').value    = '';
    document.getElementById('inTotalAlloc').value = '';
    document.getElementById('suppName').textContent = '—';
    document.getElementById('suppBal').textContent  = '—';
    document.getElementById('ftrNonAloc').textContent = '0.00';
    allBills = []; visibleBills = [];
    document.getElementById('billsTbody').innerHTML =
        '<tr><td colspan="12" style="text-align:center;padding:20px;color:#9ca3af;">Enter a supplier code to load bills.</td></tr>';
...
async function doModalSearch(q) {
    ...
    _srchTimer = setTimeout(async () => {
        try {
            const d = await post({ action: 'supplier_search', q: q.trim() });
            modalItems = d.data || [];
            modalIdx   = -1;
            const ul = document.getElementById('modalSrchList');
            if (!modalItems.length) {
                ul.innerHTML = '<li class="srch-empty">No suppliers found.</li>';
                return;
            }
...
function selectModalItem(i) {
    ...
    if (!r) return;
    document.getElementById('inSupp').value = r.code;
    closeModal();
    loadSupplier(r.code);
}
