...
@php
    ...
    $balanceLabel = trim((string) ($v['balance_label'] ?? ''));
@endphp
<div class="wrap">
    <div class="toolbar">
    ...
    <div class="doc">
        ...
        <div style="text-align:center;border-bottom:1px solid #d7deea;padding-bottom:12px;margin-bottom:14px" data-print-app-header="voucher-company">
            ...
            @if(trim((string) ($company['address1'] ?? '')) !== '')
            <div style="font-size:13px;color:#334155;margin-top:4px">{{ $company['address1'] }}</div>
            @endif
            @if(trim((string) ($company['address2'] ?? '')) !== '')
            <div style="font-size:13px;color:#334155">{{ $company['address2'] }}</div>
            @endif
            @if(trim((string) ($company['phone'] ?? '')) !== '' || trim((string) ($company['gstin'] ?? '')) !== '')
        ...
        <div class="grid">
            ...
            <div class="panel">
                <h3>Balance</h3>
                @if($showObCb)
                <div class="amount-row"><span class="label">Current Balance</span><span class="value">{{ number_format(abs($balance), 2, '.', '') }} {{ $balanceLabel }}</span></div>
                @endif
            </div>
        </div>
