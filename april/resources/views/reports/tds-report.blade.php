...
<div class="grid-wrap">
  <table>
    ...
    <tbody id="tbody"><tr><td colspan="7" style="text-align:center;padding:40px;color:#94a3b8">Press <b>Show</b> to load data</td></tr></tbody>
  </table>
</div>
...
const COLS = [
  ...
  { key:'tdsperc',   label:'TDS%',       cls:'num', fmt:'2' },
...
function renderBody(){
  ...
  let html='';
  let totTmc=0, totTds=0;

  rows.forEach(r=>{
    totTmc += r.tmc; totTds += r.tdsamt;
    html+='<tr>';
    COLS.forEach(c=>{
      if(c.date){
  ...
  document.getElementById('summary').innerHTML =
    ...
    '<span>Total MC+St.Amt: <b>'+nf(totTmc)+'</b></span>'+
    '<span>Total TDS: <b>'+nf(totTds)+'</b></span>';
}
...
function loadData(){
  ...
  fetch(SITE+'/api/tds-report/data?'+params.toString())
    .then(r=>r.json())
    .then(d=>{
