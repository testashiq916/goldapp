...
const RLEVEL = {{ $rlevel }};
let rows = [], totals = {}, sortCol = '', sortDir = 1, isRegister = false;
...
function bookHeaders(){
  return [
    ['Bill No','billno'],['Date','tdate'],['Customer','custname'],
    ['Addr','addr'],['GSTIN','tin'],['State','statecode'],
    ['Gr.Wgt','grosswgt',1,3],['Net Wgt','netwgt',1,3],
    ['VA','va',1],['Amount','amount',1],
    ['SGST','sgst',1],['CGST','cgst',1],['IGST','igst',1],
    ['Disc','discount',1],['Round','round_amt',1],['Net Amt','netamt',1]
  ];
}
...
function regHeaders(){
  return [
    ['Bill No','billno'],['Date','tdate'],['Customer','custname'],
    ['Item','itemname'],['HSN','hsn'],
    ['Rate','rate',1],['Qty','qty',1,0],['Weight','weight',1,3],
    ['St.Wgt','stonewgt',1,3],['St.Price','stoneprice',1],
    ['Net Wgt','netwgt',1,3],['VA','va',1],['Amount','amount',1]
  ];
}
...
async function loadLookups(){
  try {
    const r = await fetch(SITE+'/api/jewllery-book/lookups');
...
async function loadData(){
  isRegister = document.getElementById('cbRegister').checked;
  const qs = new URLSearchParams({
    date1: document.getElementById('date1').value,
    date2: document.getElementById('date2').value,
    rlevel: RLEVEL,
    mode: isRegister ? 'register' : 'book',
  ...
  const modeLabel = isRegister ? ' (Register)' : '';
  document.getElementById('subHeader').textContent =
    'From: '+document.getElementById('date1').value+' To: '+document.getElementById('date2').value + modeLabel;

  try {
    const r = await fetch(SITE+'/api/jewllery-book/data?'+qs);
    const d = await r.json();
...
ReportExport.init('btnSaveAs', headers, ()=>rows,
  ()=>'jewllery-book-'+document.getElementById('date1').value+'-to-'+document.getElementById('date2').value);
</script>
</body>
</html>
