...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .window{max-width:1050px;margin:12px auto;background:var(--panel);border:1px solid #8a8a8a;box-shadow:inset 0 0 0 1px #efefef}
  ...
  .top-row input[type="date"]{height:32px;padding:4px 8px;border:1px solid var(--line);background:var(--field)}
  .top-row input[type="date"]:focus{outline:none;background:#fff5e8;border-color:#9b6a18}
  .top-row label{display:flex;align-items:center;gap:5px;font-weight:700;cursor:pointer}
...
<div class="window">
  ...
  <div class="frame">
    <div class="top-row">
      ...
      <label><input type="checkbox" id="cbWithZero"> With Zero Int Amt</label>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Name</th>
            <th style="width:115px;text-align:right">Total Colln</th>
            <th style="width:80px;text-align:right">Int Rate</th>
            <th style="width:115px;text-align:right">Int Amt</th>
          </tr>
        </thead>
    ...
    <div class="footer-row">
      ...
      <span>Total Colln: <span id="totalColln">0.00</span> &nbsp;&nbsp; Total Int: <span id="totalInt">0.00</span></span>
    </div>
...
<script>
const API  = @json(url('/api/kuri-int-post'));
let gridData = [];
...
function renderGrid(){
  ...
  gridData.forEach((r, i) => {
    ...
    tr.innerHTML =
      ...
      +'<td>'+esc(r.code)+'</td>'
      +'<td>'+esc(r.name)+'</td>'
      +'<td style="text-align:right">'+fmt2(r.tcolln)+'</td>'
      +'<td style="text-align:right">'+fmt3(r.intrate)+'</td>'
      +'<td><input type="number" step="0.01" data-idx="'+i+'" value="'+fmt2(r.intamt)+'"></td>';
    body.appendChild(tr);
  });
  ...
  body.querySelectorAll('td input[type="number"]').forEach(inp => {
    inp.addEventListener('change', e => {
      gridData[+e.target.dataset.idx].intamt = parseFloat(e.target.value) || 0;
      updateTotals();
    });
  });
...
function updateTotals(){
  let colln = 0, intTotal = 0, cnt = 0;
  gridData.forEach(r => {
    if (r.sel) { colln += parseFloat(r.tcolln||0); intTotal += parseFloat(r.intamt||0); cnt++; }
  });
  $('rowCount').textContent = cnt;
  $('totalColln').textContent = fmt2(colln);
  $('totalInt').textContent = fmt2(intTotal);
}
...
$('btnCalc').addEventListener('click', async () => {
  try {
    ...
    const d = await api({
      action: 'show',
      tdate: $('tdate').value,
      with_zero: $('cbWithZero').checked
    });
    gridData = d.data || [];
    renderGrid();
    setStatus(gridData.length + ' party(ies) loaded.', true);
  } catch(e){ setStatus('Error: '+e.message); }
  finally { $('btnCalc').disabled = false; }
});

$('btnSave').addEventListener('click', async () => {
  const selected = gridData.filter(r => r.sel && parseFloat(r.intamt||0) > 0);
  if (selected.length === 0){ setStatus('No rows selected with interest amount.'); return; }
  if (!confirm('Save interest posting for ' + selected.length + ' party(ies)?')) return;

  try {
    setStatus('Saving...');
    $('btnSave').disabled = true;
    const d = await api({ action:'save', tdate: $('tdate').value, rows: selected });
...
(async () => {
  try {
    ...
    $('tdate').value = d.today || '';
    setStatus('Ready. Click Calculate to load parties.', true);
