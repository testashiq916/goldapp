...
.balance-pos{background:#d1fae5;color:#065f46;padding:2px 6px;border-radius:4px}
.balance-neg{background:#fee2e2;color:#991b1b;padding:2px 6px;border-radius:4px}
  ...
  <table>
    ...
    <tfoot>
      <tr>
        ...
        <td class="num" id="totCharge">0.00</td>
...
<div class="empty" id="emptyState">Set filters and click <strong>Show</strong> to view the A/c summary.</div>
</div>
<script>
const API = @json(url('/api/refinery-reports/ac-summary/data'));
...
document.getElementById('filterForm').addEventListener('submit', async e=>{
  ...
  try{
    ...
    document.getElementById('reportBody').innerHTML=rows.map(r=>`<tr>
      ...
      <td class="num">${n2(r.total_charge)}</td>
      <td class="num pos">${n2(r.total_paid)}</td>
      <td class="num"><span class="${r.balance>0?'balance-pos':r.balance<0?'balance-neg':''}">${n2(r.balance)}</span></td>
    </tr>`).join('');
    ...
    document.getElementById('totCharge').textContent=n2(t.total_charge);
    document.getElementById('totPaid').textContent=n2(t.total_paid);
