...
th.grp-a{background:#dbeafe}th.grp-b{background:#dcfce7}
  ...
  <table>
    <thead>
      <tr>
        ...
        <th colspan="2" class="grp-a" style="text-align:center">Issued</th>
        <th colspan="2" class="grp-b" style="text-align:center">Received</th>
        <th rowspan="2" class="num">Loss (g)</th>
        <th rowspan="2" class="num">Touch Diff</th>
        <th rowspan="2" class="num">Mud Less</th>
        <th rowspan="2" class="num">Test Pcs</th>
        <th rowspan="2" class="num">Test %</th>
        <th rowspan="2" class="num">Exp Wgt</th>
      </tr>
      <tr>
        <th class="num grp-a">Weight (g)</th><th class="num grp-a">Touch</th>
        <th class="num grp-b">Weight (g)</th><th class="num grp-b">Touch</th>
      </tr>
    </thead>
    ...
    <tfoot>
      <tr>
        ...
        <td></td>
        <td class="num" id="totLoss">0.000</td>
        <td></td>
...
<div class="empty" id="emptyState">Set filters and click <strong>Show</strong> to view the analysis.</div>
</div>
<script>
const API = @json(url('/api/refinery-reports/analysis/data'));
...
document.getElementById('filterForm').addEventListener('submit', async e=>{
  ...
  try{
    ...
    document.getElementById('reportBody').innerHTML=rows.map(r=>`<tr>
      ...
      <td><span class="badge ${r.status===2?'badge-ret':'badge-fwd'}">${esc(r.status_lbl)}</span></td>
      <td>${esc(r.refinername)}</td>
      <td>${esc(r.itemname)}</td>
      <td class="num">${r.issuedwgt>0?n3(r.issuedwgt):''}</td>
      <td class="num">${r.touch>0?n2(r.touch):''}</td>
      <td class="num">${r.rcvdwgt>0?n3(r.rcvdwgt):''}</td>
      <td class="num">${r.rcvdtouch>0?n2(r.rcvdtouch):''}</td>
      <td class="num ${r.losswgt>0?'neg':r.losswgt<0?'pos':''}">${r.losswgt!==0?n3(r.losswgt):''}</td>
      <td class="num ${r.touchdiff>0?'pos':r.touchdiff<0?'neg':''}">${r.touchdiff!==0?n2(r.touchdiff):''}</td>
    ...
    document.getElementById('totLoss').textContent=n3(t.losswgt);
