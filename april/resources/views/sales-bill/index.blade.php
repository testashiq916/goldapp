...
<div class="main-window">
  ...
  <div class="top-section">
    ...
    <div>
      ...
      <div class="row" id="rowPrevAdv"><label>Prev.Adv</label><input id="fPrevAdv" class="sm" value=".00"><span style="flex:1 1 auto;"></span><button id="btnRateAll" style="font-size:10px;padding:2px 8px;border:1px solid #cbd5e0;border-radius:3px;background:#fff;cursor:pointer;">Rate All</button></div>
    </div>
  </div>
  ...
  <div class="bottom-section">
    ...
    <div>
      ...
      <div class="row"><label>Tax Amt</label><input id="fTaxPerc" class="w50" value="0"><input id="fTaxAmt" class="w50" value=".00"></div>
  ...
  <div class="action-buttons">
    ...
    <button class="highlight" id="btnSalesReturn">Sales Return</button>
    ...
    <button id="btnQtnPrint">Qtn Print</button>
  </div>
</div>
...
(() => {
  ...
  async function applyQuotationToSale(billNo) {
    ...
    try {
      ...
      setVal('fSmName', d.salesman_code || '');
      syncSalesmanTitle();
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (res.tax_perc !== undefined) {
          setVal('fTaxPerc', String(res.tax_perc));
          triggerRecalc();
        }
      }
    } catch (e) {
  ...
  $('btnCustCreate').addEventListener('click', () => {
    const popupUrl = siteUrl + '/customer?type=C';
    window.open(
      popupUrl,
      'goldapp_customer_create',
      'width=1200,height=760,left=120,top=60,resizable=yes,scrollbars=yes'
    );
  });
  ...
  function recalcMainByPb(idx, changedCol = '') {
    ...
    if (changedCol === 'amount') {
      ...
      if (USE_VA_MODEL) {
        if (r.manual_mcharge && !['itemcode', 'vaperc', 'rate'].includes(changedCol)) {
  ...
  $('itemsTbody').addEventListener('keydown', async function(e) {
    ...
    if (curClass === 'ic-code') {
      ...
      if (ok && cur && cur.item_code && cur.item_name) {
        ...
        renderItemsTable();
        setTimeout(() => {
          const qty = $('itemsTbody').querySelector(`.ic-qty[data-idx="${idx}"]`);
  ...
  $('itemsTbody').addEventListener('keydown', async function(e) {
    ...
    if (el.classList.contains('ic-code')) {
      ...
      if (ok && cur && cur.item_code && cur.item_name) {
        ...
        // After item lookup, always continue with qty entry first.
        renderItemsTable();
        setTimeout(() => {
          const qty = $('itemsTbody').querySelector(`.ic-qty[data-idx="${idx}"]`);
          if (qty) { qty.focus(); qty.select(); }
  ...
  async function saveBill() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (mode === 'bill') {
          showAlert((isQuotation ? 'Quotation saved: ' : 'Bill saved: ') + billNo, true, async () => {
            if (isQuotation) {
            ...
            if (!isQuotation) {
              try {
                await maybePromptEInvoice(billNo, savedNetTotal);
              } catch (_) {}
            }
            pendingResetAfterPrint = false;
            setTimeout(() => {
  ...
  async function loadBill(billNo) {
    try {
      ...
      const ex = d.extra || {};
      let loadedTaxPerc = Number(ex.tax_perc ?? NaN);
      if (!Number.isFinite(loadedTaxPerc)) loadedTaxPerc = NaN;
      if ((!Number.isFinite(loadedTaxPerc) || loadedTaxPerc <= 0) && Number(ex.tax || 0) > 0) {
        const taxableBase = Number(d.bill_total || 0);
        if (taxableBase > 0) {
          loadedTaxPerc = ((Number(ex.tax || 0) * 100) / taxableBase);
        }
      }
      if (!Number.isFinite(loadedTaxPerc) || loadedTaxPerc < 0) loadedTaxPerc = 0;
      setVal('fDiscount', toNum(ex.discount) > 0 ? fmt2(ex.discount) : '');
      setVal('fDiscountPerc', ex.discount_perc ?? 0);
      setVal('fTaxAmt', fmt2(ex.tax));
  ...
  async function resetToAddMode(prefillBillNo = '', prefillTaxPerc = null, savedBillNo = '') {
    ...
    addItemRow();
    applyHeaderRate();
    if (prefillTaxPerc !== null && prefillTaxPerc !== undefined && prefillTaxPerc !== '') {
      setVal('fTaxPerc', String(prefillTaxPerc));
    }
    triggerRecalc();
    syncSalesmanTitle();
