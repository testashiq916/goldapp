  ...
  <div class="title">
    <div id="shopInfo" data-print-app-header="goldsmith-shop">
      @if(!empty($company['SHOPNM']))
        ...
        @if(!empty($company['SHOPADDR']))<div>{{ $company['SHOPADDR'] }}</div>@endif
        @if(!empty($company['SHOPPHONE']) || !empty($company['KGST']))
          <div>{{ trim(($company['SHOPPHONE'] ?? '') . (!empty($company['KGST']) ? '  GSTIN: '.$company['KGST'] : '')) }}</div>
        @endif
      @endif
    </div>
